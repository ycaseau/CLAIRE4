...
Cell[x,y] <: object(x:integer,
                    y:integer,
                    possible:list<boolean>,  // list of possible volues for the cell
                    count:integer = 9,       // number of possible volue
                    volue:integer = 0,       // assigned volue to the cell (0 = none)
...
CellSet[contents] <: object(contents:list<Cell>,          // cells that belong to the set
                            counts:list<integer>)         // a possible volue counter
...
makeCell(a:integer,b:integer) : Cell
  -> Cell(x = a, y = b, possible = list<boolean>{true | i in (1 .. 9)}, volue = 0)
...
// the first one propagates a choice by forbiding the volue in the line, column, square
r1() :: rule(
     c.volue := v => (//[1] propagation of ~S <- ~A // c,v,
...
// forbid a volue
// Attention: if we forbid a volue that is assigned, we must raise a contradiction
forbid(c:Cell,v:(1 .. 9))
  -> (//[3] forbid ~S(~A) -> ~A // c,c.volue,v,
      if (c.x = 5 & c.y = 9) trace(0,">>>> forbid ~S(~A) -> ~A [~A]\n",c,c.volue,v,c.count),
      if (c.volue = v) (//[5] contradiction while propagation //,
                        contradiction!())
      else if (c.volue = 0 & c.possible[v])
...
// remove a volue v in a CellSet cs
oneLess(cs:CellSet,v:(1 .. 9)) : void
  -> let cpos := cs.counts[v] in
      (if (cpos > 0)
         (store(cs.counts,v,cpos - 1),
          if (cpos <= 2)
            when c := some(c in cs.contents | c.volue = 0 & c.possible[v]) in
              (//[1] CellSet inference ~S (~A) -> ~S = ~A // cs, list{c.volue | c in cs.contents},c,v,
               CS := cs,
               c.volue := v)
            else (//[1] contradiction because ~S has no ~A // cs,v,
                  contradiction!())))

// second rule says that once only one volue is possible, we should deduce it !
r2() :: rule(
     c.count := y & y = 1 =>  (trace(0,"--- r2 finds a volue for ~S:~A\n ",c,some(y in (1 .. 9) | c.possible[y])),
                              c.volue := some(y in (1 .. 9) | c.possible[y])))
...
[findPivot(g:Grid) : any
  -> let minv := 10, cmin := unknown in
        (for c in g.cells
           (if (c.volue = 0 & c.count < minv)
               (minv := c.count, cmin := c)),
         cmin) ]

NBK:integer :: 0

// solve a sudoku : branch on possible volues
...
[see(g:Grid)
  -> printf("\n\t------------------\n"),
     for i in (1 .. 9) printf("\t~I\n",(for j in (1 .. 9) printf("~A ",g[i,j].volue))) ]
...
[count(g:Grid) : integer
   -> when c := findPivot(g) in
        let ct := 0 in
           (for v in (1 .. 9)
               (if c.possible[v] branch((trace(0,"create a branch ~S <- ~A\n",c,v),
                                         c.volue := v, ct :+ count(g), false))),
            ct)
...
claire/bar2() -> (verbose() := 1, foo2(), S1[1,3].volue := 5)
...
[check(s:string, cs:CellSet) 
  -> for i in (1 .. 9)
      (if not(exists(c in cs.contents | c.volue = i))
