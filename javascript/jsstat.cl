...
// in CLAIRE v4.1.14, we now support the case where x is not a functional expression     
[update_simple(c:js_producer,r:relation,x:any,y:any) : void
   -> //[0] update simple with x = ~S, y = ~S // x,y,
      if (j_func(x) & j_func(y))
         (if r.multivalued? 
              printf("~I.push(~I)",c_member(c, x, any,r),j_expression(y))
           else printf("~I = ~I",c_member(c, x, any,r),j_expression(y)))
      else if j_func(y)
         let varg1 :=  build_Variable("var_updatex", domain(r)), nv1 := c_string(PRODUCER,varg1) in     // create a unique variable  
         (var_declaration(PRODUCER,nv1,1),                   
          call_j_statement(x,nv1,false),
          update_simple(c,r,varg1,y))
     else let varg2 := build_Variable("var_updatey", range(r)), nv2 := c_string(PRODUCER,varg2) in     // create a unique variable  
         (var_declaration(PRODUCER,nv2,1),                   
          call_j_statement(y,nv2,false),
          update_simple(c,r,x,varg2)) ]
...
// we now assume that the test returns a boolean !
 [j_statement(self:If,v:string,loop:any) : void
   ->  if (self.other = assert) nil   // Note: should be garded by compiler.optimize? (but it currently does not work)
       else (if j_func(self.test)                                       // easy case 
            ...
            if (self.other = nil | self.other = assert | (self.other = false &  v = "niet")) close_block("If")
