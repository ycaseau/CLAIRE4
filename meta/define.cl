...
self_eval(self:Defmethod) : any
 -> (if not(self.arg % Call)
     ...
     let p := make_a_property(self.arg.selector),
       ...
       (if (length(l) > 10) error("[110] method in CLAIRE cannot have more than 10 arguments: ~S", l),
...
self_eval(self:Defarray) : any
 -> (let a := (self.arg as Call).args,
       ...
       (write(range, ar, extract_pattern(self.set_arg, nil)),
        ...
        if (unknown?(d) & (ar.range <= integer | ar.range <= float) & not(e % lambda))
            trace(0,"=== CLAIRE4.1 Warning: unknown not allowed as a default for table ~S with range ~S\n ",ar,ar.range),
        if known?(d) 
