/***** CLAIRE Compilation of file /Users/ycaseau/Dropbox/src/clairev4.12/src/meta/call.cl 
...
/* The go function for: occurbreak(self:any) [status=0] */
func F_Language_occurbreak_any (self *ClaireAny) *ClaireBoolean { 
  var Result *ClaireBoolean
  if (self.Isa.IsIn(C_list) == CTRUE) { 
    { var g0119 *ClaireList = ToList(self)
      { var arg_1 *ClaireAny
        { 
          ...
          for i_it := 0; i_it < y_len; i_it++ { 
            y = y_support.At(i_it)
            if (F_Language_occurbreak_any(y) == CTRUE) { 
              arg_1 = CTRUE.Id()
              break
              } 
            } 
    ...
    }  else if (self.Isa.IsIn(C_Return) == CTRUE) { 
    Result = CTRUE
    }  else if (self.Isa.IsIn(C_Instruction) == CTRUE) { 
    { var g0121 *ClaireInstruction = To_Instruction(self)
      { var arg_2 *ClaireAny
        { 
          var s *ClaireSlot
          _ = s
          ...
          arg_2= CFALSE.Id()
          var s_support *ClaireList
          s_support = g0121.Id().Isa.Slots
          for _,s_iter = range(s_support.ValuesO()){ 
            s = ToSlot(s_iter)
            if (F_Language_occurbreak_any(Core.F_get_slot(s,ToObject(g0121.Id()))) == CTRUE) { 
              arg_2 = CTRUE.Id()
              break
              } 
            } 
...
// The EID go function for: occurbreak @ any (throw: false) 
func E_Language_occurbreak_any (self EID) EID { 
  return EID{F_Language_occurbreak_any(ANY(self) ).Id(),0}} 
...
func F_instruction_copy_any (self *ClaireAny) *ClaireAny { 
  var Result *ClaireAny
  if (self.Isa.IsIn(C_list) == CTRUE) { 
    { var g0123 *ClaireList = ToList(self)
      { var l *ClaireList = g0123.Copy()
        { var i int = 1
          { var g0124 int = g0123.Length()
            for (i <= g0124) { 
              ToArray(l.Id()).NthPut(i,F_instruction_copy_any(g0123.At(i-1)))
              i = (i+1)
              } 
            } 
          } 
    ...
    { var g0126 *ClaireInstruction = To_Instruction(self)
      { var o *ClaireInstruction = To_Instruction(g0126.Copy().Id())
        { 
          var s *ClaireSlot
          _ = s
          var s_iter *ClaireAny
          var s_support *ClaireList
          s_support = g0126.Id().Isa.Slots
          for _,s_iter = range(s_support.ValuesO()){ 
            s = ToSlot(s_iter)
            Core.F_put_slot(s,ToObject(o.Id()),F_instruction_copy_any(Core.F_get_slot(s,ToObject(g0126.Id()))))
            } 
          } 
        Result = o.Id()
        } 
