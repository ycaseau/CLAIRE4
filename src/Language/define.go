...
// CLAIRE 4: fast definition when no close nor fancy slots
...
func F_Language_fast_definition_ask_class (c *ClaireClass) *ClaireBoolean { 
  ...
  { 
    ...
    v_and1 = Core.F__sup_integer(c.Open,1)
    if (v_and1 == CFALSE) {Result = CFALSE
    } else { 
      v_and1 = Equal(C_method.Id(),Core.F_owner_any(Core.F__at_property1(C_close,c).Id()).Id()).Not
      if (v_and1 == CFALSE) {Result = CFALSE
      } else { 
        { var arg_1 *ClaireAny
          { 
          ...
          { var y *ClaireAny
            ...
            { var s *ClaireObject = Core.F__at_property1(p,self.Isa)
              if (C_slot.Id() == s.Isa.Id()) { 
                { var g0223 *ClaireSlot = ToSlot(s.Id())
                  if (y == CNULL) { 
                    lp = lp.AddFast(p.Id())
                    } 
                  if (g0223.Range.Contains(y) != CTRUE) { 
                    loop_1 = Core.F_range_is_wrong_slot(g0223,y)
                    } else {
  ...
  { var _Zc *ClaireClass = self.Arg
    { var _Zo *ClaireObject = ToObject(CNULL)
      ...
      if (_Zc.IsIn(C_thing) == CTRUE) { 
        if ((Core.F_owner_any(self.Ident.Value()).IsIn(Core.C_global_variable) == CTRUE) && 
            (Equal(Core.ToGlobalVariable(self.Ident.Value()).Range.Id(),CEMPTY.Id()) == CTRUE)) { 
          Core.F_tformat_string(MakeString("CLAIRE4 Warning: global constant defined twice: ~S\n"),0,MakeConstantList(self.Id()))
          } 
        var try_1 EID
        ...
        if (_Zo.Isa.IsIn(C_property) == CTRUE) { 
          { var g0225 *ClaireProperty = ToProperty(_Zo.Id())
            if (g0225.Restrictions.Length() > 0) { 
              Result = ToException(Core.C_general_error.Make(MakeString("[188] the property ~S is already defined").Id(),MakeConstantList(g0225.Id()).Id())).Close()
              } else {
              Result = EID{CFALSE.Id(),0}
              } 
            } 
        ...
        for i_it := 0; i_it < x_len; i_it++ { 
          ...
          { var v *ClaireAny = CNULL
            if (x.Isa.IsIn(C_Call) == CTRUE) { 
              { var g0226 *Call = To_Call(x)
                var try_3 EID
                try_3 = EVAL(g0226.Args.At(1))
                ...
                v = ANY(try_3)
                loop_2 = v.ToEID()
                g0226 = To_Call(g0226.Args.At(0))
                loop_2 = EID{g0226.Id(),0}
                }
          ...
          { var lrange *ClaireList
            ...
            { var lbody *ClaireList
              ...
              { var m *ClaireMethod = F_add_method_property(p,lp,ToType(lrange.At(0)),ToInteger(lbody.At(0)).Value,ToFunction(lbody.At(1)))
                if (l.Length() > 10) { 
                  Result = ToException(Core.C_general_error.Make(MakeString("[110] method in CLAIRE cannot have more than 10 arguments: ~S").Id(),MakeConstantList(l.Id()).Id())).Close()
                  } else {
                  Result = EID{CFALSE.Id(),0}
                  } 
                if !ErrorIn(Result) {
                if ((p.Open > 0) && 
                  ...
                  { var rtest *ClaireAny
                    { var r_some *ClaireAny = CNULL
                      { 
                        ...
                        for _,r_iter = range(p.Restrictions.ValuesO()){ 
                          ...
                          if (r.Id() != m.Id()) { 
                            if (Core.F__exp_list(r.Domain,m.Domain).Length() != 0) { 
                              r_some = r.Id()
                              break
                              } 
                            } 
                    ...
                    if (rtest != CNULL) { 
                      { var r *ClaireRestriction = ToRestriction(rtest)
                        Core.F_tformat_string(MakeString("--- WARNING ! [186] conflict between ~S and ~S is dangerous since ~S is closed\n"),1,MakeConstantList(m.Id(),r.Id(),p.Id()))
                        } 
                      } else {
                ...
                if (lbody.At(2) != C_body.Id()) { 
                  ...
                  { 
                    ...
                    va_arg1 = m
                    var try_5 EID
                    { var arg_6 *ClaireLambda
                      var try_7 EID
                      try_7 = F_lambda_I_list(lv,lbody.At(2))
                      if ErrorIn(try_7) {try_5 = try_7
                      } else {
                      arg_6 = ToLambda(OBJ(try_7))
                      try_5 = F_Language_jito_any(arg_6.Id())
                      }
                      } 
                    if ErrorIn(try_5) {Result = try_5
                    } else {
                    va_arg2 = ToLambda(OBJ(try_5))
  ...
  { var n int = 0
    { 
      ...
      for CLcount := 0; CLcount < v_list2.Length(); CLcount++{ 
        v = To_Variable(v_list2.At(CLcount))
        var try_1 EID
...
func F_extract_pattern_any (x *ClaireAny,path *ClaireList) EID { 
  ...
  if (C_class.Id() == x.Isa.Id()) { 
    { var g0228 *ClaireClass = ToClass(x)
      Result = EID{g0228.Id(),0}
      } 
    }  else if (C_set.Id() == x.Isa.Id()) { 
    { var g0229 *ClaireSet = ToSet(x)
      { var z *ClaireAny
        var try_1 EID
        if (g0229.Size() == 1) { 
          { var arg_2 *ClaireAny
            var try_3 EID
            try_3 = Core.F_the_type(ToType(g0229.Id()))
        ...
        if (z.Isa.IsIn(C_Reference) == CTRUE) { 
          { var g0230 *ClaireReference = To_Reference(z)
            { var w *ClaireReference = To_Reference(g0230.Copy().Id())
              w.Arg = CTRUE
              Result = EID{w.Id(),0}
              } 
            } 
          } else {
          Result = EID{g0229.Id(),0}
          } 
        }
        } 
      } 
    }  else if (x.Isa.IsIn(C_Tuple) == CTRUE) { 
    { var g0232 *Tuple = To_Tuple(x)
      { var ltp *ClaireList
        var try_4 EID
        { 
          ...
          v_list4 = g0232.Args
          try_4 = EID{CreateList(ToType(CEMPTY.Id()),v_list4.Length()).Id(),0}
          for CLcount := 0; CLcount < v_list4.Length(); CLcount++{ 
            z = v_list4.At(CLcount)
            var try_5 EID
            ...
            v_local4 = ANY(try_5)
            ToList(OBJ(try_4)).PutAt(CLcount,v_local4)
            } 
          }
          } 
        if ErrorIn(try_4) {Result = try_4
    ...
    { var g0233 *Core.GlobalVariable = Core.ToGlobalVariable(x)
      Result = F_extract_pattern_any(g0233.Value,path)
      } 
    }  else if (x.Isa.IsIn(C_Call) == CTRUE) { 
    { var g0234 *Call = To_Call(x)
      { var p *ClaireProperty = g0234.Selector
        if (p.Id() == Core.C_U.Id()) { 
          { var x1 *ClaireAny
            var try_7 EID
            try_7 = F_extract_pattern_any(g0234.Args.At(0),CNIL)
            if ErrorIn(try_7) {Result = try_7
            } else {
            x1 = ANY(try_7)
            { var x2 *ClaireAny
              var try_8 EID
              try_8 = F_extract_pattern_any(g0234.Args.At(1),CNIL)
          ...
          { var arg_9 *ClaireAny
            var try_11 EID
            try_11 = F_extract_pattern_any(g0234.Args.At(0),CNIL)
            if ErrorIn(try_11) {Result = try_11
            } else {
            arg_9 = ANY(try_11)
            { var arg_10 *ClaireAny
              var try_12 EID
              try_12 = F_extract_pattern_any(g0234.Args.At(1),CNIL)
          ...
          { var v1 *ClaireAny = F_extract_item_any(g0234.Args.At(0),CNIL.Id())
            { var v2 *ClaireAny = F_extract_item_any(g0234.Args.At(1),CNIL.Id())
          ...
          Result = F_extract_pattern_nth_list(g0234.Args,path)
          }  else if (p.Id() == C__star.Id()) { 
          { var z *ClaireAny
            var try_13 EID
            try_13 = F_extract_pattern_any(g0234.Args.At(0),path)
    ...
    { var g0235 *ClaireType = ToType(x)
      Result = EID{g0235.Id(),0}
      } 
    }  else if (x.Isa.IsIn(C_unbound_symbol) == CTRUE) { 
    { var g0236 *ClaireUnboundSymbol = ToUnboundSymbol(x)
      { var s *ClaireSymbol
        var try_14 EID
        try_14 = F_extract_symbol_any(g0236.Id())
        ...
        { var v *ClaireAny
          ...
          if (v != CNULL) { 
            ...
            if (path.Isa.IsIn(C_list) == CTRUE) { 
              g0240I = Core.F__sup_integer(path.Length(),1)
              } else {
              g0240I = CFALSE
              } 
            if (g0240I == CTRUE) { 
      ...
      { var l1 *ClaireAny = l.At(1)
        { var l2 *ClaireList = ToList(OBJ(Core.F_CALL(C_args,ARGS(l.At(2).ToEID()))))
          { var l3 *ClaireList = ToType(C_any.Id()).EmptyList()
            { var n int = 1
              { var g0241 int = INT(Core.F_CALL(C_length,ARGS(l1.ToEID())))
                Result= EID{CFALSE.Id(),0}
                for (n <= g0241) { 
                  var loop_3 EID
                  _ = loop_3
                        ...
                        { var g0242 *Set = To_Set(y)
                          { var v *ClaireAny
                            ...
                            { var arg_8 *ClaireList
                              ...
                              try_7 = F_extract_pattern_any(g0242.Args.At(0),arg_8)
                              }
                              } 
                            if ErrorIn(try_7) {try_6 = try_7
                            } else {
                            ...
                            if (v == C_void.Id()) { 
                              ...
                              { var g0244 *ClaireReference = To_Reference(v)
                                { var z *ClaireReference = To_Reference(g0244.Copy().Id())
                                  z.Arg = CTRUE
                                  try_6 = EID{z.Id(),0}
                                  } 
                                } 
...
{ 
  ...
  v_and0 = MakeBoolean((self.Id() == C_list.Id()) || (self.Id() == C_set.Id()) || (self.Id() == C_subtype.Id()))
  if (v_and0 == CFALSE) {try_1 = EID{CFALSE.Id(),0}
  } else { 
    v_and0 = Equal(MakeInteger(l.Length()).Id(),MakeInteger(1).Id())
    if (v_and0 == CFALSE) {try_1 = EID{CFALSE.Id(),0}
    } else { 
      var try_2 EID
      { var y *ClaireAny = l.At(0)
        { var z *ClaireAny
          ...
          if (y.Isa.IsIn(Core.C_global_variable) == CTRUE) { 
            { var g0247 *Core.GlobalVariable = Core.ToGlobalVariable(y)
              g0247 = Core.ToGlobalVariable(OBJ(Core.F_CALL(C_value,ARGS(l.At(0).ToEID()))))
              y = g0247.Id()
              } 
            } 
          { 
            ...
            v_or5 = z.Isa.IsIn(C_type)
            if (v_or5 == CTRUE) {try_2 = EID{CTRUE.Id(),0}
            } else { 
              v_or5 = Equal(self.Id(),C_subtype.Id())
              if (v_or5 == CTRUE) {try_2 = EID{CTRUE.Id(),0}
              } else { 
                if (y.Isa.IsIn(C_Call) == CTRUE) { 
                  { var g0248 *Call = To_Call(y)
                    v_or5 = MakeBoolean((g0248.Selector.Id() != C__equal.Id()) || (g0248.Args.Length() != 2))
                    } 
      ...
      if (l.At(0).Isa.IsIn(C_Do) == CTRUE) { 
        { var v_out *ClaireList = ToType(CEMPTY.Id()).EmptyList()
          { 
            ...
            v_support = ToList(OBJ(Core.F_CALL(C_args,ARGS(l.At(0).ToEID()))))
            ...
            for (n <= g0250) { 
              ...
              { var y *ClaireAny = l.At(n-1)
                { var p *ClaireAny = CNULL
                  { var v *ClaireAny = CNULL
                    if (y.Isa.IsIn(C_Call) == CTRUE) { 
                      { var g0251 *Call = To_Call(y)
                        if ((g0251.Selector.Id() != C__equal.Id()) || 
                            (g0251.Args.Length() != 2)) { 
                          loop_4 = ToException(Core.C_general_error.Make(MakeString("[114] Wrong parametrization ~S").Id(),MakeConstantList(g0251.Id()).Id())).Close()
                          } else {
                        ...
                        var try_5 EID
                        try_5 = F_make_a_property_any(g0251.Args.At(0))
                        ...
                        { var _CL_obj *Set = To_Set(new(Set).Is(C_Set))
                          _CL_obj.Args = MakeConstantList(g0251.Args.At(1))
                          v = _CL_obj.Id()
                          } 
                        loop_4 = v.ToEID()
                        }}
                      ...
                      { var g0252 *Vardef = To_Vardef(y)
                        var try_6 EID
                        try_6 = F_make_a_property_any(g0252.Pname.Id())
...
{ var arg_1 *ClaireBoolean
  if (x.Isa.IsIn(C_Call) == CTRUE) { 
    { var g0255 *Call = To_Call(x)
      arg_1 = MakeBoolean((g0255.Selector.Id() == C_nth.Id()) && (g0255.Args.At(0) == C_type.Id()))
      } 
    } else {
    arg_1 = CFALSE
    } 
  ...
  { var f *ClaireAny
    ...
    if (x.Isa.IsIn(C_Call) == CTRUE) { 
      { var g0258 *Call = To_Call(x)
        g0263I = Equal(g0258.Selector.Id(),C_function_I.Id())
        } 
      } else {
      g0263I = CFALSE
      } 
    ...
    if (x.Isa.IsIn(C_And) == CTRUE) { 
      { var g0259 *And = To_And(x)
        { var y *ClaireAny = g0259.Args.At(0)
          var g0264I *ClaireBoolean
          if (y.Isa.IsIn(C_Call) == CTRUE) { 
            { var g0260 *Call = To_Call(y)
              g0264I = Equal(g0260.Selector.Id(),C_function_I.Id())
              } 
            } else {
            g0264I = CFALSE
            } 
          if (g0264I == CTRUE) { 
            f = y
            g0259 = To_And(g0259.Args.At(1))
            } 
          } 
        x = g0259.Id()
        } 
      }  else if (x.Isa.IsIn(C_Call) == CTRUE) { 
      { var g0261 *Call = To_Call(x)
        if (g0261.Selector.Id() == C_function_I.Id()) { 
          g0261 = To_Call(C_body.Id())
          } 
        x = g0261.Id()
        } 
      } else {
...
if (x.Isa.IsIn(C_list) == CTRUE) { 
  { var g0265 *ClaireList = ToList(x)
    { 
      var v_list2 *ClaireList
      var y *ClaireAny
      var v_local2 *ClaireAny
      v_list2 = g0265
      Result = ToType(CreateList(ToType(CEMPTY.Id()),v_list2.Length()).Id())
      for CLcount := 0; CLcount < v_list2.Length(); CLcount++{ 
        y = v_list2.At(CLcount)
        v_local2 = F_type_I_any(y).Id()
        ToList(Result.Id()).PutAt(CLcount,v_local2)
        } 
      } 
    } 
  }  else if (x.Isa.IsIn(C_Param) == CTRUE) { 
  { var g0266 *ClaireParam = To_Param(x)
    { var _CL_obj *ClaireParam = To_Param(new(ClaireParam).Is(C_Param))
      _CL_obj.Arg = g0266.Arg
      _CL_obj.Params = g0266.Params
      { 
        ...
        { 
          ...
          var v_local4 *ClaireAny
          v_list4 = g0266.Args
          va_arg2 = CreateList(ToType(CEMPTY.Id()),v_list4.Length())
          for CLcount := 0; CLcount < v_list4.Length(); CLcount++{ 
            y = v_list4.At(CLcount)
            v_local4 = F_type_I_any(y).Id()
            va_arg2.PutAt(CLcount,v_local4)
            } 
          } 
        va_arg1.Args = va_arg2
        } 
          ...
          { var d *ClaireAny
            ...
            if ((d == CNULL) && 
                (((ar.Range.Included(ToType(C_integer.Id())) == CTRUE) || 
                  ...
                  (e.Isa.IsIn(C_lambda) != CTRUE))) { 
              Core.F_tformat_string(MakeString("=== CLAIRE4.1 Warning: unknown not allowed as a default for table ~S with range ~S\n "),0,MakeConstantList(ar.Id(),ar.Range.Id()))
              } 
            if (d != CNULL) { 
            ...
            if (a.Length() == 2) { 
              ...
              if (s.Isa.IsIn(C_Interval) == CTRUE) { 
                { var g0272 *ClaireInterval = To_Interval(s.Id())
                  ar.Params = MakeInteger((g0272.Arg1-1)).Id()
                  ar.Graph = Core.F_typed_copy_list_type(ar.Range,Core.F_size_Interval(g0272),d).Id()
                  } 
                } else {
                ar.Params = C_any.Id()
                ar.GraphInit()
              ...
              if (e.Isa.IsIn(C_lambda) == CTRUE) { 
                { var g0274 *ClaireLambda = ToLambda(e)
                  { 
                    var y *ClaireAny
                    _ = y
                    Result= EID{CFALSE.Id(),0}
                    ...
                    for i_it := 0; i_it < y_len; i_it++ { 
                      ...
                      { var arg_13 *ClaireAny
                        var try_14 EID
                        try_14 = Core.F_funcall_lambda1(g0274,y)
              ...
              { var s2 *ClaireTypeExpression
                ...
                if (e.Isa.IsIn(C_lambda) == CTRUE) { 
                  { var g0276 *ClaireLambda = ToLambda(e)
                    { 
                      var y1 *ClaireAny
                      _ = y1
                      Result= EID{CFALSE.Id(),0}
                        ...
                        { 
                          ...
                          for i_it := 0; i_it < y2_len; i_it++ { 
                            ...
                            { var arg_36 *ClaireAny
                              var try_37 EID
                              try_37 = Core.F_CALL(C_funcall,ARGS(EID{g0276.Id(),0},y1.ToEID(),y2.ToEID()))
...
if (self.Args.At(0) != ClEnv.Id()) { 
  ...
  { var _Zcondition *ClaireAny = self.Arg
    { var ru *ClaireAny = self.Ident.Value()
      ...
      { var g0279 *ClaireTuple
        var try_1 EID
        try_1 = F_make_filter_any(_Zcondition)
        if ErrorIn(try_1) {Result = try_1
        } else {
        g0279 = ToTuple(OBJ(try_1))
        { var R *ClaireAny = ToList(g0279.Id()).At(0)
          { var lvar *ClaireAny = ToList(g0279.Id()).At(1)
            { var d *LanguageDemon
              var try_2 EID
              { var arg_3 *ClaireAny
                var try_4 EID
              ...
              if (R.Isa.IsIn(C_property) == CTRUE) { 
                { var g0281 *ClaireProperty = ToProperty(R)
                  g0282I = Equal(MakeInteger(g0281.Restrictions.Length()).Id(),MakeInteger(0).Id())
                  } 
                } else {
                g0282I = CFALSE
                } 
...
var Result *ClaireBoolean
if (r.Isa.IsIn(C_property) == CTRUE) { 
  { var g0283 *ClaireProperty = ToProperty(r.Id())
    { var arg_1 *ClaireAny
      { 
        ...
        var x_iter *ClaireAny
        arg_1= CFALSE.Id()
        for _,x_iter = range(g0283.Restrictions.ValuesO()){ 
...
{ var c *ClaireAny
  ...
  if (c.Isa.IsIn(C_Call) == CTRUE) { 
    { var g0286 *Call = To_Call(c)
      g0290I = MakeBoolean(((g0286.Selector.Id() == Core.C_write.Id()) || 
          (g0286.Selector.Id() == C_nth_equal.Id())) && (g0286.Args.At(0).Isa.IsIn(C_relation) == CTRUE))
      } 
    } else {
    g0290I = CFALSE
    } 
  if (g0290I == CTRUE) { 
      ...
      { var x *ClaireVariable
        ...
        { var y1 *ClaireAny = ToList(OBJ(Core.F_CALL(C_args,ARGS(c.ToEID())))).At(2)
          ...
          if (y1.Isa.IsIn(C_Call) == CTRUE) { 
            { var g0287 *Call = To_Call(y1)
              g0291I = Equal(g0287.Selector.Id(),C__inf_dash.Id())
              } 
            } else {
            g0291I = CFALSE
            } 
    ...
    if (c.Isa.IsIn(C_Call) == CTRUE) { 
      { var g0288 *Call = To_Call(c)
        g0292I = MakeBoolean((g0288.Selector.Id() == C_add.Id()) && (g0288.Args.At(0).Isa.IsIn(C_relation) == CTRUE))
        } 
      } else {
      g0292I = CFALSE
      } 
    if (g0292I == CTRUE) { 
      ...
      if (c.Isa.IsIn(C_Call) == CTRUE) { 
        { var g0289 *Call = To_Call(c)
          g0293I = Equal(MakeInteger(g0289.Args.Length()).Id(),MakeInteger(2).Id())
          } 
        } else {
        g0293I = CFALSE
        } 
    ...
    { var _Ztest *ClaireAny
      ...
      { var _Zbody *ClaireAny = conc
        ...
        if (F_eventMethod_ask_relation2(R) == CTRUE) { 
          if (cond.Isa.IsIn(C_And) == CTRUE) { 
            { var g0294 *And = To_And(cond)
              var try_1 EID
              if (g0294.Args.Length() > 2) { 
...
if (x.Isa.IsIn(C_property) == CTRUE) { 
  { var g0298 *ClaireProperty = ToProperty(x.Id())
    var g0302I *ClaireBoolean
    { var arg_1 *ClaireAny
      { 
        ...
        var s_iter *ClaireAny
        arg_1= CFALSE.Id()
        for _,s_iter = range(g0298.Restrictions.ValuesO()){ 
          s = ToRestriction(s_iter)
          var g0303I *ClaireBoolean
          { var arg_2 *ClaireBoolean
            if (C_slot.Id() == s.Isa.Id()) { 
              { var g0299 *ClaireSlot = ToSlot(s.Id())
                arg_2 = g0299.Range.Contains(g0299.Default)
                } 
              } else {
              arg_2 = CFALSE
              } 
  ...
  { var g0300 *ClaireTable = ToTable(x.Id())
    if (g0300.Range.Contains(g0300.Default) == CTRUE) { 
      Result = g0300.Range
      } else {
      Result = ToType(C_any.Id())
      } 
...
if (ClEnv.Jito_ask != CTRUE) { 
  ...
  { var g0304 *ClaireList = ToList(self)
    { 
      var x *ClaireAny
      _ = x
      Result= EID{CFALSE.Id(),0}
  ...
  { var g0305 *Vardef = To_Vardef(self)
    g0305.Isa = C_Variable
    Result = EID{g0305.Id(),0}
    } 
  }  else if (self.Isa.IsIn(C_lambda) == CTRUE) { 
  { var g0306 *ClaireLambda = ToLambda(self)
    Result = F_Language_jito_any(g0306.Body)
    if !ErrorIn(Result) {
    Result = EID{g0306.Id(),0}
    }
    } 
  }  else if (self.Isa.IsIn(C_And) == CTRUE) { 
  { var g0307 *And = To_And(self)
    Result = F_Language_jito_any(g0307.Args.Id())
    } 
  }  else if (self.Isa.IsIn(C_Or) == CTRUE) { 
  { var g0308 *Or = To_Or(self)
    Result = F_Language_jito_any(g0308.Args.Id())
    } 
  }  else if (self.Isa.IsIn(C_Call) == CTRUE) { 
  { var g0309 *Call = To_Call(self)
    Result = g0309.MakeJito()
    if !ErrorIn(Result) {
    Result = EID{CTRUE.Id(),0}
    }
    } 
  }  else if (self.Isa.IsIn(C_Let) == CTRUE) { 
  { var g0310 *Let = To_Let(self)
    Result = g0310.LetJito()
    } 
  }  else if (self.Isa.IsIn(C_Assign) == CTRUE) { 
  { var g0311 *Assign = To_Assign(self)
    if (g0311.ClaireVar.Isa.IsIn(C_Variable) != CTRUE) { 
      Result = ToException(Core.C_general_error.Make(MakeString("[101] ~S is not a variable but a ~S").Id(),MakeConstantList(g0311.ClaireVar,g0311.ClaireVar.Isa.Id()).Id())).Close()
      } else {
      Result = EID{CFALSE.Id(),0}
      } 
    if !ErrorIn(Result) {
  ...
  { var g0312 *Gassign = To_Gassign(self)
    if (g0312.ClaireVar.Range.Contains(g0312.ClaireVar.Value) == CTRUE) { 
  ...
  { var g0313 *Do = To_Do(self)
    Result = F_Language_jito_any(g0313.Args.Id())
    } 
  }  else if (self.Isa.IsIn(C_If) == CTRUE) { 
  { var g0314 *If = To_If(self)
    Result = F_Language_jito_any(g0314.Arg)
    if !ErrorIn(Result) {
    Result = F_Language_jito_any(g0314.Test)
    if !ErrorIn(Result) {
    Result = F_Language_jito_any(g0314.Other)
    }}
    } 
  }  else if (self.Isa.IsIn(C_Iteration) == CTRUE) { 
  { var g0315 *Iteration = To_Iteration(self)
    { var v *ClaireVariable = g0315.ClaireVar
      { var s *ClaireAny = g0315.SetArg
        { var o_ask *ClaireBoolean
          { 
            var v_and5 *ClaireBoolean
            
            if (s.Isa.IsIn(C_Call) == CTRUE) { 
              { var g0316 *Call = To_Call(s)
                v_and5 = Equal(g0316.Selector.Id(),C__dot_dot.Id())
                } 
              } else {
              v_and5 = CFALSE
              } 
            if (v_and5 == CFALSE) {o_ask = CFALSE
            } else { 
              v_and5 = MakeBoolean((v.Range.Id() == CNULL))
              if (v_and5 == CFALSE) {o_ask = CFALSE
              } else { 
                o_ask = CTRUE} 
              } 
          ...
          { var arg_2 *ClaireList
            ...
            { 
              var v_bag_arg *ClaireAny
              try_3= EID{ToType(CEMPTY.Id()).EmptyList().Id(),0}
              ToList(OBJ(try_3)).AddFast(g0315.Id())
              var try_4 EID
              try_4 = F_static_type_any(g0315.SetArg)
              if ErrorIn(try_4) {try_3 = try_4
              } else {
              v_bag_arg = ANY(try_4)
              ToList(OBJ(try_3)).AddFast(v_bag_arg)}
  ...
  { var g0317 *While = To_While(self)
    Result = F_Language_jito_any(g0317.Test)
    if !ErrorIn(Result) {
    Result = F_Language_jito_any(g0317.Arg)
    }
    } 
  }  else if (self.Isa.IsIn(C_Construct) == CTRUE) { 
  { var g0318 *Construct = To_Construct(self)
    Core.F_tformat_string(MakeString("-- Construct jito: ~S\n"),3,MakeConstantList(g0318.Id()))
    Result = F_Language_jito_any(g0318.Args.Id())
    } 
  }  else if (self.Isa.IsIn(C_Exists) == CTRUE) { 
  { var g0319 *Exists = To_Exists(self)
    Result = F_Language_jito_any(g0319.SetArg)
    if !ErrorIn(Result) {
    Result = F_Language_jito_any(g0319.Arg)
    if !ErrorIn(Result) {
    Result = F_Language_jito_any(g0319.Other)
    }}
    } 
  }  else if (self.Isa.IsIn(C_Handle) == CTRUE) { 
  { var g0320 *ClaireHandle = To_ClaireHandle(self)
    if (C_class.Id() != g0320.Test.Isa.Id()) { 
      Result = ToException(Core.C_general_error.Make(MakeString("syntax: [try %S] must use a class").Id(),MakeConstantList(g0320.Test).Id())).Close()
      } else {
      Result = EID{CFALSE.Id(),0}
      } 
    if !ErrorIn(Result) {
    Result = F_Language_jito_any(g0320.Arg)
    if !ErrorIn(Result) {
    Result = F_Language_jito_any(g0320.Other)
    }}
    } 
  }  else if (self.Isa.IsIn(C_Definition) == CTRUE) { 
  { var g0321 *Definition = To_Definition(self)
    if (F_Language_fast_definition_ask_class(g0321.Arg) == CTRUE) { 
      { 
    ...
    { var n int = larg.Length()
      { var m *ClaireAny = CNULL
        ...
        { 
          ...
          v_and4 = Equal(p.Id(),Core.C_write.Id())
          if (v_and4 == CFALSE) {g0327I = CFALSE
          } else { 
            { var p2 *ClaireAny = self.Args.At(0)
              if (p2.Isa.IsIn(C_property) == CTRUE) { 
                { var g0323 *ClaireProperty = ToProperty(p2)
                  v_and4 = MakeBoolean((g0323.Inverse.Id() == CNULL) && (g0323.Store_ask != CTRUE) && (g0323.IfWrite == CNULL))
                  } 
                } else {
                v_and4 = CFALSE
                } 
        ...
        var g0328I *ClaireBoolean
        { 
          var v_and4 *ClaireBoolean
          
          v_and4 = Core.F__inf_equal_integer(p.Open,1)
          if (v_and4 == CFALSE) {g0328I = CFALSE
          } else { 
            v_and4 = Core.F__inf_equal_integer(p.Restrictions.Length(),12)
            if (v_and4 == CFALSE) {g0328I = CFALSE
            } else { 
              { var arg_1 *ClaireAny
                { 
        ...
        if (g0328I == CTRUE) { 
          { var lt *ClaireList
            ...
            { 
              ...
              var v_local6 *ClaireAny
              v_list6 = larg
              try_4 = EID{CreateList(ToType(CEMPTY.Id()),v_list6.Length()).Id(),0}
              for CLcount := 0; CLcount < v_list6.Length(); CLcount++{ 
                ...
                v_local6 = ANY(try_5)
                ToList(OBJ(try_4)).PutAt(CLcount,v_local6)
                } 
              }
              } 
            if ErrorIn(try_4) {Result = try_4
        ...
        if (C_method.Id() == m.Isa.Id()) { 
          { var g0325 *ClaireMethod = ToMethod(m)
            g0330I = MakeBoolean((g0325.Functional.Id() == CNULL)).Not
            } 
          } else {
          g0330I = CFALSE
          } 
...
{ var n int = lt.Length()
  { var ld *ClaireList = x.Domain
    { 
      ...
      v_and2 = Equal(MakeInteger(ld.Length()).Id(),MakeInteger(n).Id())
