        ...
        for i_it := 0; i_it < v_len; i_it++ { 
          ...
          if (v.Isa.IsIn(C_Variable) == CTRUE) /* If:5 */{ 
            /* Let:6 */{ 
              ...
              /* noccur = 1 */
              void_try5 = F_ppvariable_Variable(g0001)
              /* Let-6 */} 
            } else {
...
func F_lambda_I_list (lvar *ClaireList ,self *ClaireAny ) EID { 
    ...
    /* Let:2 */{ 
      var corps *ClaireAny  
      /* noccur = 1 */
      var corps_try00043 EID 
      corps_try00043 = F_lexical_build_any(self,lvar,ToInteger(C__starvariable_index_star.Value).Value)
      /* ERROR PROTECTION INSERTED (corps-Result) */
      if ErrorIn(corps_try00043) {Result = corps_try00043
      } else {
      corps = ANY(corps_try00043)
      /* Let:3 */{ 
...
func F_lexical_build_any (self *ClaireAny ,lvar *ClaireList ,n int) EID { 
    ...
    if ((self.Isa.IsIn(C_thing) == CTRUE) || 
      ...
      if (self.Isa.IsIn(C_Variable) == CTRUE) /* If:3 */{ 
        /* Let:4 */{ 
          var g0005 *ClaireVariable   = To_Variable(self)
          /* noccur = 3 */
          if (Equal(MakeInteger(g0005.Index).Id(),CNULL) == CTRUE) /* If:5 */{ 
            Result = ToException(Core.C_general_error.Make(MakeString("[145] the symbol ~A is unbound").Id(),MakeConstantList(g0005.Pname.Id()).Id())).Close()
            } else {
            Result = EID{CFALSE.Id(),0}
            /* If-5 */} 
          /* ERROR PROTECTION INSERTED (Result-Result) */
          if !ErrorIn(Result) {
          Result = EID{g0005.Id(),0}
          }
          /* Let-4 */} 
        /* If!3 */}  else if (self.Isa.IsIn(C_Call) == CTRUE) /* If:3 */{ 
        /* Let:4 */{ 
          var g0006 *Call   = To_Call(self)
          /* noccur = 6 */
          /* Let:5 */{ 
            var s *ClaireAny  
            /* noccur = 2 */
            var s_try00106 EID 
            s_try00106 = F_lexical_change_any(g0006.Selector.Id(),lvar)
            /* ERROR PROTECTION INSERTED (s-Result) */
            if ErrorIn(s_try00106) {Result = s_try00106
            } else {
            s = ANY(s_try00106)
            Result = F_lexical_build_any(g0006.Args.Id(),lvar,n)
            /* ERROR PROTECTION INSERTED (Result-Result) */
            if !ErrorIn(Result) {
            if (g0006.Selector.Id() != s) /* If:6 */{ 
              g0006.Selector = Core.C_call
              /* update:7 */{ 
                var va_arg1 *Call  
                var va_arg2 *ClaireList  
                va_arg1 = g0006
                va_arg2 = F_cons_any(s,g0006.Args)
        ...
        /* Let:4 */{ 
          var g0007 *ClaireInstruction   = To_Instruction(self)
          /* noccur = 4 */
          /* Let:5 */{ 
            var _Ztype *ClaireClass   = g0007.Isa
            /* noccur = 2 */
            if (C_Instruction_with_var.Descendents.Contain_ask(_Ztype.Id()) == CTRUE) /* If:6 */{ 
              Result = Core.F_put_property2(C_mClaire_index,ToObject(OBJ(Core.F_CALL(C_var,ARGS(EID{g0007.Id(),0})))),MakeInteger(n).Id())
              /* ERROR PROTECTION INSERTED (Result-Result) */
              if !ErrorIn(Result) {
              n = (n+1)
            ...
            /* For:6 */{ 
              ...
              for _,s = range(_Ztype.Slots.ValuesO())/* loop:7 */{ 
                ...
                _ = void_try8
                /* Let:8 */{ 
                  var x *ClaireAny   = Core.F_get_slot(ToSlot(s),ToObject(g0007.Id()))
                  /* noccur = 4 */
                  if (((x.Isa.IsIn(C_thing) == CTRUE) || 
                        (x.Isa.IsIn(C_unbound_symbol) == CTRUE)) && 
                      (ToRestriction(s).Range.Id() == C_any.Id())) /* If:9 */{ 
                    /* Let:10 */{ 
                      var g0011UU *ClaireAny  
                      /* noccur = 1 */
                      var g0011UU_try001211 EID 
                      g0011UU_try001211 = F_lexical_change_any(x,lvar)
                      /* ERROR PROTECTION INSERTED (g0011UU-void_try8) */
                      if ErrorIn(g0011UU_try001211) {void_try8 = g0011UU_try001211
                      } else {
                      g0011UU = ANY(g0011UU_try001211)
                      void_try8 = Core.F_put_slot(ToSlot(s),ToObject(g0007.Id()),g0011UU).ToEID()
                      }
                      /* Let-10 */} 
                    } else {
        ...
        /* Let:4 */{ 
          var g0008 *ClaireList   = ToList(self)
          /* noccur = 3 */
          /* Let:5 */{ 
            var _Zn int  = g0008.Length()
            /* noccur = 5 */
            Result= EID{CFALSE.Id(),0}
            for (_Zn > 0) /* while:6 */{ 
              var void_try7 EID 
              ...
              { 
              /* Let:7 */{ 
                var x *ClaireAny   = g0008.At(_Zn-1)
                /* noccur = 4 */
                if ((x.Isa.IsIn(C_thing) == CTRUE) || 
                    (x.Isa.IsIn(C_unbound_symbol) == CTRUE)) /* If:8 */{ 
                  /* Let:9 */{ 
                    var g0013UU *ClaireAny  
                    /* noccur = 1 */
                    var g0013UU_try001410 EID 
                    g0013UU_try001410 = F_lexical_change_any(x,lvar)
                    /* ERROR PROTECTION INSERTED (g0013UU-void_try7) */
                    if ErrorIn(g0013UU_try001410) {void_try7 = g0013UU_try001410
                    } else {
                    g0013UU = ANY(g0013UU_try001410)
                    void_try7 = ToArray(g0008.Id()).NthPut(_Zn,g0013UU).ToEID()
                    }
                    /* Let-9 */} 
                  } else {
...
func F_lexical_change_any (self *ClaireAny ,lvar *ClaireList ) EID { 
    ...
    /* Let:2 */{ 
      ...
      /* Let:3 */{ 
        ...
        /* noccur = 1 */
        var _Zname_try00174 EID 
        if (self.Isa.IsIn(C_Variable) == CTRUE) /* If:4 */{ 
          /* Let:5 */{ 
            var g0015 *ClaireVariable   = To_Variable(self)
            /* noccur = 1 */
            _Zname_try00174 = EID{g0015.Pname.Id(),0}
            /* Let-5 */} 
          } else {
          _Zname_try00174 = F_extract_symbol_any(self)
          /* If-4 */} 
        /* ERROR PROTECTION INSERTED (_Zname-Result) */
        if ErrorIn(_Zname_try00174) {Result = _Zname_try00174
        } else {
        _Zname = ToSymbol(OBJ(_Zname_try00174))
        /* For:4 */{ 
          var x *ClaireAny  
          _ = x
...
func F_extract_symbol_any (self *ClaireAny ) EID { 
    ...
    if (self.Isa.IsIn(C_unbound_symbol) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        ...
        /* noccur = 1 */
        Result = EID{g0018.Name.Id(),0}
        /* Let-3 */} 
      /* If!2 */}  else if (self.Isa.IsIn(C_thing) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        var g0019 *ClaireThing   = ToThing(self)
        /* noccur = 1 */
        Result = EID{g0019.Name.Id(),0}
        /* Let-3 */} 
      /* If!2 */}  else if (C_class.Id() == self.Isa.Id()) /* If:2 */{ 
      /* Let:3 */{ 
        var g0020 *ClaireClass   = ToClass(self)
        /* noccur = 1 */
        Result = EID{g0020.Name.Id(),0}
        /* Let-3 */} 
      /* If!2 */}  else if (self.Isa.IsIn(C_symbol) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        var g0021 *ClaireSymbol   = ToSymbol(self)
        /* noccur = 1 */
        Result = EID{g0021.Id(),0}
        /* Let-3 */} 
      /* If!2 */}  else if (self.Isa.IsIn(C_Variable) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        var g0022 *ClaireVariable   = To_Variable(self)
        /* noccur = 1 */
        Result = EID{g0022.Pname.Id(),0}
        /* Let-3 */} 
      /* If!2 */}  else if (C_boolean.Id() == self.Isa.Id()) /* If:2 */{ 
      /* Let:3 */{ 
        var g0023 *ClaireBoolean   = ToBoolean(self)
        /* noccur = 1 */
        if (g0023 == CTRUE) /* If:4 */{ 
...
func F_make_a_property_any (self *ClaireAny ) EID { 
    ...
    if (self.Isa.IsIn(Core.C_global_variable) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        var g0025 *Core.GlobalVariable   = Core.ToGlobalVariable(self)
        /* noccur = 1 */
        Result = F_make_a_property_any(g0025.Value)
        /* Let-3 */} 
      /* If!2 */}  else if (self.Isa.IsIn(C_property) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        var g0026 *ClaireProperty   = ToProperty(self)
        /* noccur = 1 */
        Result = EID{g0026.Id(),0}
        /* Let-3 */} 
      /* If!2 */}  else if (self.Isa.IsIn(C_symbol) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        var g0027 *ClaireSymbol   = ToSymbol(self)
        /* noccur = 3 */
        /* Let:4 */{ 
          var x *ClaireAny   = g0027.Get()
          /* noccur = 4 */
          if (x.Isa.IsIn(C_property) == CTRUE) /* If:5 */{ 
            /* Let:6 */{ 
              var g0028 *ClaireProperty   = ToProperty(x)
              /* noccur = 1 */
              Result = F_make_a_property_any(g0028.Id())
              /* Let-6 */} 
            /* If!5 */}  else if (x.Isa.IsIn(Core.C_global_variable) == CTRUE) /* If:5 */{ 
            /* Let:6 */{ 
              var g0029 *Core.GlobalVariable   = Core.ToGlobalVariable(x)
              /* noccur = 1 */
              Result = F_make_a_property_any(g0029.Value)
              /* Let-6 */} 
            } else {
            /* Let:6 */{ 
              ...
              /* noccur = 5 */
              var p_try00337 EID 
              p_try00337 = new(ClaireProperty).IsNamed(C_property,g0027).ToEID()
              /* ERROR PROTECTION INSERTED (p-Result) */
              if ErrorIn(p_try00337) {Result = p_try00337
              } else {
              p = ToProperty(OBJ(p_try00337))
              p.Comment = g0027.String_I()
      ...
      /* Let:3 */{ 
        var g0031 *ClaireUnboundSymbol   = ToUnboundSymbol(self)
        /* noccur = 1 */
        Result = F_make_a_property_any(g0031.Name.Id())
        /* Let-3 */} 
      } else {
...
func F_put_buffer_void () EID { 
    ...
    /* Let:2 */{ 
      ...
      /* noccur = 1 */
      var buffer_try00343 EID 
      buffer_try00343 = Core.F_end_of_string_void()
      /* ERROR PROTECTION INSERTED (buffer-Result) */
      if ErrorIn(buffer_try00343) {Result = buffer_try00343
      } else {
      buffer = ToString(OBJ(buffer_try00343))
...
func F_printexp_any (self *ClaireAny ,comp *ClaireBoolean ) EID { 
    var Result EID 
    var g0039I *ClaireBoolean  
    /* or:2 */{ 
      var v_or2 *ClaireBoolean  
      
      if (self.Isa.IsIn(C_Call) == CTRUE) /* If:3 */{ 
        /* Let:4 */{ 
          var g0038 *Call   = To_Call(self)
          /* noccur = 2 */
          v_or2 = MakeBoolean((g0038.Selector.Isa.IsIn(C_operation) == CTRUE) && (comp != CTRUE) && (g0038.Args.Length() == 2)).Not
          /* Let-4 */} 
        } else {
        v_or2 = CFALSE
        /* If-3 */} 
      if (v_or2 == CTRUE) {g0039I = CTRUE
      } else /* or:3 */{ 
...
func F_pretty_print_any (self *ClaireAny ) EID { 
    ...
    Core.C_pretty.Pprint = CFALSE
    /* Let:2 */{ 
      var g0040UU *ClaireString  
      /* noccur = 1 */
      var g0040UU_try00413 EID 
      g0040UU_try00413 = Core.F_end_of_string_void()
      /* ERROR PROTECTION INSERTED (g0040UU-Result) */
      if ErrorIn(g0040UU_try00413) {Result = g0040UU_try00413
      } else {
      g0040UU = ToString(OBJ(g0040UU_try00413))
      F_princ_string(g0040UU)
      Result = EVOID
      }
      /* Let-2 */} 
    }
...
func F_static_type_any (self *ClaireAny ) EID { 
    ...
    if (self.Isa.IsIn(C_Variable) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        var g0042 *ClaireVariable   = To_Variable(self)
        /* noccur = 1 */
        Result = EID{g0042.Range.Class_I().Id(),0}
        /* Let-3 */} 
      /* If!2 */}  else if (self.Isa.IsIn(Core.C_global_variable) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        var g0043 *Core.GlobalVariable   = Core.ToGlobalVariable(self)
        /* noccur = 2 */
        /* Let:4 */{ 
          var r *ClaireType   = g0043.Range
          /* noccur = 2 */
          if (F_boolean_I_any(r.Id()) == CTRUE) /* If:5 */{ 
            Result = EID{r.Class_I().Id(),0}
            } else {
            Result = EID{g0043.Value.Isa.Id(),0}
            /* If-5 */} 
          /* Let-4 */} 
        /* Let-3 */} 
      ...
      /* Let:3 */{ 
        var g0047 *Call_plus   = To_Call_plus(self)
        /* noccur = 2 */
        /* Let:4 */{ 
          var p *ClaireProperty   = g0047.Selector
          /* noccur = 2 */
          /* Let:5 */{ 
            var s *ClaireObject  
            /* noccur = 2 */
            var s_try00696 EID 
            /* Let:6 */{ 
              var g0070UU *ClaireClass  
              /* noccur = 1 */
              var g0070UU_try00717 EID 
              g0070UU_try00717 = F_static_type_any(g0047.Args.At(1-1))
              /* ERROR PROTECTION INSERTED (g0070UU-s_try00696) */
              if ErrorIn(g0070UU_try00717) {s_try00696 = g0070UU_try00717
              } else {
              g0070UU = ToClass(OBJ(g0070UU_try00717))
              s_try00696 = EID{Core.F__at_property1(p,g0070UU).Id(),0}
              }
              /* Let-6 */} 
            /* ERROR PROTECTION INSERTED (s-Result) */
            if ErrorIn(s_try00696) {Result = s_try00696
            } else {
            s = ToObject(OBJ(s_try00696))
            if (C_slot.Id() == s.Isa.Id()) /* If:6 */{ 
              /* Let:7 */{ 
                var g0048 *ClaireSlot   = ToSlot(s.Id())
                /* noccur = 1 */
                Result = EID{g0048.Range.Id(),0}
                /* Let-7 */} 
              } else {
      ...
      /* Let:3 */{ 
        var g0050 *CallSlot   = To_CallSlot(self)
        /* noccur = 1 */
        /* Let:4 */{ 
          var s *ClaireSlot   = g0050.Selector
          /* noccur = 4 */
          /* Let:5 */{ 
            ...
            /* For:6 */{ 
              ...
              for _,s2 = range(p.Definition.ValuesO())/* loop:7 */{ 
                if (C_slot.Id() == s2.Isa.Id()) /* If:8 */{ 
                  /* Let:9 */{ 
                    var g0051 *ClaireSlot   = ToSlot(s2)
                    /* noccur = 2 */
                    if (ToType(Core.F_domain_I_restriction(ToRestriction(s.Id())).Id()).Included(ToType(Core.F_domain_I_restriction(ToRestriction(g0051.Id())).Id())) == CTRUE) /* If:10 */{ 
                      s = g0051
                      /* If-10 */} 
                    /* Let-9 */} 
                  /* If-8 */} 
      ...
      /* Let:3 */{ 
        var g0052 *CallMethod   = To_CallMethod(self)
        /* noccur = 3 */
        /* Let:4 */{ 
          var p *ClaireProperty   = g0052.Arg.Selector
          /* noccur = 1 */
          if (p.Id() == C_nth.Id()) /* If:5 */{ 
            Result = F_Language_static_type_nth_any(g0052.Args.At(1-1))
            } else {
            Result = EID{g0052.Arg.Range.Class_I().Id(),0}
            /* If-5 */} 
          /* Let-4 */} 
        /* Let-3 */} 
      /* If!2 */}  else if (self.Isa.IsIn(C_Call) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        var g0053 *Call   = To_Call(self)
        /* noccur = 2 */
        /* Let:4 */{ 
          var p *ClaireProperty   = g0053.Selector
          /* noccur = 2 */
          if (p.Id() == C_nth.Id()) /* If:5 */{ 
            Result = F_Language_static_type_nth_any(g0053.Args.At(1-1))
            } else {
      ...
      /* Let:3 */{ 
        var g0054 *Assign   = To_Assign(self)
        /* noccur = 1 */
        Result = Core.F_CALL(C_Language_s_type,ARGS(g0054.Arg.ToEID()))
        /* Let-3 */} 
      /* If!2 */}  else if (self.Isa.IsIn(C_Let) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        var g0055 *Let   = To_Let(self)
        /* noccur = 1 */
        Result = F_static_type_any(g0055.Arg)
        /* Let-3 */} 
      /* If!2 */}  else if (self.Isa.IsIn(C_Do) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        var g0056 *Do   = To_Do(self)
        /* noccur = 1 */
        /* Let:4 */{ 
          var g0072UU *ClaireAny  
          /* noccur = 1 */
          var g0072UU_try00735 EID 
          g0072UU_try00735 = Core.F_last_list(g0056.Args)
          /* ERROR PROTECTION INSERTED (g0072UU-Result) */
          if ErrorIn(g0072UU_try00735) {Result = g0072UU_try00735
          } else {
          g0072UU = ANY(g0072UU_try00735)
          Result = F_static_type_any(g0072UU)
          }
          /* Let-4 */} 
        /* Let-3 */} 
      /* If!2 */}  else if (self.Isa.IsIn(C_If) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        var g0057 *If   = To_If(self)
        /* noccur = 2 */
        /* Let:4 */{ 
          var g0074UU *ClaireClass  
          /* noccur = 1 */
          var g0074UU_try00765 EID 
          g0074UU_try00765 = F_static_type_any(g0057.Arg)
          /* ERROR PROTECTION INSERTED (g0074UU-Result) */
          if ErrorIn(g0074UU_try00765) {Result = g0074UU_try00765
          } else {
          g0074UU = ToClass(OBJ(g0074UU_try00765))
          /* Let:5 */{ 
            var g0075UU *ClaireClass  
            /* noccur = 1 */
            var g0075UU_try00776 EID 
            g0075UU_try00776 = F_static_type_any(g0057.Other)
            /* ERROR PROTECTION INSERTED (g0075UU-Result) */
            if ErrorIn(g0075UU_try00776) {Result = g0075UU_try00776
            } else {
            g0075UU = ToClass(OBJ(g0075UU_try00776))
            Result = EID{Core.F_meet_class(g0074UU,g0075UU).Id(),0}
            }
            /* Let-5 */} 
          }
          /* Let-4 */} 
      ...
      /* Let:3 */{ 
        var g0065 *Exists   = To_Exists(self)
        /* noccur = 1 */
        if (g0065.Other == CNULL) /* If:4 */{ 
      ...
      /* Let:3 */{ 
        var g0066 *Definition   = To_Definition(self)
        /* noccur = 1 */
        Result = EID{g0066.Arg.Id(),0}
        /* Let-3 */} 
...
func F_Language_static_type_nth_any (x *ClaireAny ) EID { 
    ...
    if (x.Isa.IsIn(C_Variable) == CTRUE) /* If:2 */{ 
      /* Let:3 */{ 
        var g0078 *ClaireVariable   = To_Variable(x)
        /* noccur = 1 */
        /* Let:4 */{ 
          var s *ClaireType   = g0078.Range
          /* noccur = 2 */
          if (s.Isa.IsIn(C_Param) == CTRUE) /* If:5 */{ 
            /* Let:6 */{ 
              var g0079 *ClaireParam   = To_Param(s.Id())
              /* noccur = 2 */
              if (g0079.Params.At(1-1) == C_of.Id()) /* If:7 */{ 
                /* Let:8 */{ 
                  var g0082UU *ClaireAny  
                  /* noccur = 1 */
                  var g0082UU_try00839 EID 
                  g0082UU_try00839 = Core.F_the_type(ToType(g0079.Args.At(1-1)))
                  /* ERROR PROTECTION INSERTED (g0082UU-Result) */
                  if ErrorIn(g0082UU_try00839) {Result = g0082UU_try00839
                  } else {
                  g0082UU = ANY(g0082UU_try00839)
                  Result = EID{ToTypeExpression(g0082UU).Class_I().Id(),0}
                  }
                  /* Let-8 */} 
                } else {
                Result = EID{C_any.Id(),0}
