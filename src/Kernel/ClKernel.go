...
// Note: MakeFunctionX() is used by the CLAIRE compiler (goexp.cl) to create the go function associated
// with a CLAIRE method (arity from 1 to 10).. hence the 10 variants ClaireFunctionX with arity 1 to 10
...
type ClaireFunction8 struct {
	ClaireFunction
	call eFunc8
}

func MakeFunction8(f eFunc8, name string) *ClaireFunction {
	o := new(ClaireFunction8)
	o.Isa = C_function
	o.call = f
	o.name = name
	o.arity = 8
...
type ClaireFunction9 struct {
	ClaireFunction
	call eFunc9
}

func MakeFunction9(f eFunc9, name string) *ClaireFunction {
	o := new(ClaireFunction9)
	o.Isa = C_function
	o.call = f
	o.name = name
	o.arity = 9
...
type ClaireFunction10 struct {
	ClaireFunction
	call eFunc10
}

func MakeFunction10(f eFunc10, name string) *ClaireFunction {
	o := new(ClaireFunction10)
	o.Isa = C_function
	o.call = f
	o.name = name
	o.arity = 10
...
func F_stack_apply_function(f *ClaireFunction, i int, top int) EID {
	...
	if top == i+1 {
		...
		x = ((*ClaireFunction8)(unsafe.Pointer(f))).call(ClEnv.EvalStack[i], ClEnv.EvalStack[i+1], ClEnv.EvalStack[i+2],
			ClEnv.EvalStack[i+3], ClEnv.EvalStack[i+4], ClEnv.EvalStack[i+5],ClEnv.EvalStack[i+6], ClEnv.EvalStack[i+7])
	} else if top == i+9 {
		x = ((*ClaireFunction9)(unsafe.Pointer(f))).call(ClEnv.EvalStack[i], ClEnv.EvalStack[i+1], ClEnv.EvalStack[i+2],
			ClEnv.EvalStack[i+3], ClEnv.EvalStack[i+4], ClEnv.EvalStack[i+5],ClEnv.EvalStack[i+6], ClEnv.EvalStack[i+7],
			ClEnv.EvalStack[i+8])
	} else if top == i+10 {
	    x = ((*ClaireFunction10)(unsafe.Pointer(f))).call(ClEnv.EvalStack[i], ClEnv.EvalStack[i+1], ClEnv.EvalStack[i+2],
			 ClEnv.EvalStack[i+3], ClEnv.EvalStack[i+4], ClEnv.EvalStack[i+5],ClEnv.EvalStack[i+6], ClEnv.EvalStack[i+7],
			 ClEnv.EvalStack[i+8], ClEnv.EvalStack[i+9])
	} else {
		fmt.Printf("(CLAIRE Design Limit) stack apply used from %d to %d\n",i,top)
		panic("CLAIRE does not handle more than 10 parameters in stack apply")
	}
	ClEnv.Index = i
