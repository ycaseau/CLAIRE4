/***** CLAIRE Compilation of file /Users/ycaseau/Dropbox/src/clairev4.05/src/meta/method.cl 
              ...
              { var arg_2 *ClaireBoolean
                { var l2 *ClaireList = r.Domain
                  { 
                    ...
                    v_and9 = Equal(C_class.Id(),ToTypeExpression(l2.ValuesO()[0]).Isa.Id())
                    if (v_and9 == CFALSE) {arg_2 = CFALSE
                    } else { 
                      v_and9 = Equal(MakeInteger(l2.Length()).Id(),MakeInteger(n).Id())
                      if (v_and9 == CFALSE) {arg_2 = CFALSE
                      } else { 
                        v_and9 = F__I_equal_any(l2.ValuesO()[0],C_listargs.Id())
                        if (v_and9 == CFALSE) {arg_2 = CFALSE
                        } else { 
                          { var arg_3 *ClaireAny
                            { var i int = 2
...
func F_uniform_property (p *ClaireProperty) *ClaireBoolean { 
    ...
    { 
      ...
      if (v_and2 == CFALSE) {Result = CFALSE
        ...
        v_and2 = F_uniform_restriction(ToRestriction(p.Restrictions.ValuesO()[0]))
        if (v_and2 == CFALSE) {Result = CFALSE
        } else { 
          Result = CTRUE} 
        } 
          ...
          for (i <= g0032) { 
            { var l2 *ClaireList = ToRestriction(l.At(i-1)).Domain
              if (F_tmatch_ask_list(x.Domain,l2) == CTRUE) { 
                ...
                F_tformat_string(MakeString("~S and ~S are conflicting"),2,MakeConstantList(l.At(0),x.Id()))
                } 
              } 
            i = (i+1)
            } 
