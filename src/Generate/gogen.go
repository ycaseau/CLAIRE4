...
//| CLAIRE                                                      |
//| gogen.cl                                                    |
//| Copyright (C) 2020 - 2025 Yves Caseau. All Rights Reserved  |
...
func (c *GenerateGoProducer) GlobalVar (x *Core.GlobalVariable)  { 
  ...
  if (Optimize.F_Compile_nativeVar_ask_global_variable(x) != CTRUE) { 
    PRINC(".Value")
    } 
  } 
...
// do not forget optimized (native) global variables
...
func F_imported_function_ask_any (f *ClaireAny) *ClaireBoolean { 
  ...
  if (C_function.Id() == f.Isa.Id()) { 
    { var g0042 *ClaireFunction = ToFunction(f)
      Result = Equal(MakeChar(F_string_I_function(g0042).At(1)).Id(),MakeChar('#').Id())
      } 
    } else {
    Result = CFALSE
    } 
...
func F_Generate_goMethod_ask_any (m *ClaireAny) *ClaireBoolean { 
  ...
  if (ToBoolean(ToGenerateGoProducer(Optimize.C_PRODUCER.Value).KernelMethods.Contain_ask(m).Id()) == CTRUE) { 
    ...
    { var g0044 *ClaireMethod = ToMethod(m)
      { var firstc rune = g0044.Selector.Name.String_I().At(1)
        { var _Zsig *ClaireList = F_Generate_go_signature_method(g0044)
          { var c *ClaireClass = ToClass(_Zsig.ValuesO()[0])
            { 
              var v_and6 *ClaireBoolean
              
              v_and6 = Core.F__inf_equal_char('A',firstc)
              if (v_and6 == CFALSE) {Result = CFALSE
              } else { 
                v_and6 = Core.F__inf_equal_char(firstc,'z')
                if (v_and6 == CFALSE) {Result = CFALSE
                } else { 
                  v_and6 = Core.F__I_equal_any(MakeChar(firstc).Id(),MakeChar('^').Id())
                  if (v_and6 == CFALSE) {Result = CFALSE
                  } else { 
                    v_and6 = MakeBoolean((ToType(c.Id()).Included(ToType(C_object.Id())) == CTRUE) || (c.Id() == C_port.Id()) || (c.Id() == C_environment.Id()))
                    if (v_and6 == CFALSE) {Result = CFALSE
                    } else { 
                      v_and6 = Equal(g0044.Module_I.Id(),c.Name.Defined().Id())
                      if (v_and6 == CFALSE) {Result = CFALSE
                      } else { 
                        v_and6 = MakeBoolean((g0044.Selector.IfWrite == CNULL))
                        if (v_and6 == CFALSE) {Result = CFALSE
                        } else { 
                          v_and6 = MakeBoolean((g0044.Functional.Id() == CNULL) || (F_imported_function_ask_any(g0044.Functional.Id()) != CTRUE))
                          if (v_and6 == CFALSE) {Result = CFALSE
                          } else { 
                            { var arg_1 *ClaireAny
                              { 
                                ...
                                arg_1= CFALSE.Id()
                                for _,m2_iter = range(g0044.Selector.Restrictions.ValuesO()){ 
                                  m2 = ToRestriction(m2_iter)
                                  var g0048I *ClaireBoolean
                                  { var arg_2 *ClaireBoolean
                                    if (C_method.Id() == m2.Isa.Id()) { 
                                      { var g0045 *ClaireMethod = ToMethod(m2.Id())
                                        if (C_class.Id() != g0045.Domain.ValuesO()[0].Isa.Id()) { 
                                          arg_2 = CFALSE
                                          }  else if ((g0045.Module_I.Id() == g0044.Module_I.Id()) && 
                                            (Equal(Core.F__exp_type(ToType(c.Id()),ToType(g0045.Domain.ValuesO()[0])).Id(),CEMPTY.Id()) != CTRUE)) { 
                                          arg_2 = F_Generate_arg_match_list(F_Generate_go_signature_method(g0045),_Zsig)
                                          } else {
                                          arg_2 = CTRUE
                                          } 
                                        } 
                                      } else {
                                      arg_2 = MakeBoolean((g0044.Selector.Name.Defined().Id() == C_Kernel.Id()) && (g0044.Module_I.Id() == C_Kernel.Id()))
                                      } 
                                    g0048I = arg_2.Not
                                    } 
                                  if (g0048I == CTRUE) { 
            ...
            { 
              ...
              for _,m2_iter = range(m2_support.ValuesO()){ 
                ...
                { var arg_10 *ClaireBoolean
                  ...
                  if !ErrorIn(try_11) {
                  if (C_method.Id() == m2.Isa.Id()) { 
                    { var g0049 *ClaireMethod = ToMethod(m2.Id())
                      if ((g0049.Module_I.Id() == ANY(Core.F_CALL(C_module_I,ARGS(m.ToEID())))) && 
                          (Equal(Core.F__exp_type(ToType(c.Id()),ToType(g0049.Domain.ValuesO()[0])).Id(),CEMPTY.Id()) != CTRUE)) { 
                        try_11 = EID{F_Generate_arg_match_list(F_Generate_go_signature_method(g0049),_Zsig).Id(),0}
                        } else {
...
func F_Generate_arg_match_list (l1 *ClaireList,l2 *ClaireList) *ClaireBoolean { 
  ...
  { var n int = l1.Length()
    { 
      ...
      v_and2 = Equal(MakeInteger(l2.Length()).Id(),MakeInteger(n).Id())
...
func F_Generate_retreive_method_any (p *ClaireAny,lf *ClaireAny) EID { 
  ...
  if (p.Isa.IsIn(C_property) == CTRUE) { 
    { var g0053 *ClaireProperty = ToProperty(p)
      { var m *ClaireObject
        var try_1 EID
        { var arg_2 *ClaireAny
          var try_3 EID
          ...
          arg_2 = ANY(try_3)
          try_1 = Core.F_CALL(ToProperty(Core.C__at.Id()),ARGS(EID{g0053.Id(),0},arg_2.ToEID()))
          }
          } 
        if ErrorIn(try_1) {Result = try_1
        } else {
        ...
        if (C_method.Id() == m.Isa.Id()) { 
          { var g0054 *ClaireMethod = ToMethod(m.Id())
            Result = EID{g0054.Id(),0}
            } 
          } else {
          Result = ToException(Core.C_general_error.Make(MakeString("there is no method ~S @ ~S").Id(),MakeConstantList(g0053.Id(),lf).Id())).Close()
          } 
        }
        } 
      } 
...
func F_Generate_retreive_list_any (x *ClaireAny) EID { 
  ...
  if (x.Isa.IsIn(C_type) == CTRUE) { 
    { var g0057 *ClaireType = ToType(x)
      Result = EID{g0057.Id(),0}
      } 
    }  else if (C_integer.Id() == x.Isa.Id()) { 
    { var g0058 int = ToInteger(x).Value
      Result = EID{C__INT,IVAL(g0058)}
      } 
    }  else if (x.Isa.IsIn(C_property) == CTRUE) { 
    { var g0059 *ClaireProperty = ToProperty(x)
      Result = EID{g0059.Id(),0}
      } 
    }  else if (x.Isa.IsIn(Core.C_global_variable) == CTRUE) { 
    { var g0060 *Core.GlobalVariable = Core.ToGlobalVariable(x)
    ...
    { var g0061 *Language.List = Language.To_List(x)
      { 
    ...
    { var g0062 *Language.Tuple = Language.To_Tuple(x)
      { var arg_2 *ClaireList
        ...
        { 
          ...
          v_list4 = g0062.Args
          try_3 = EID{CreateList(ToType(CEMPTY.Id()),v_list4.Length()).Id(),0}
          for CLcount := 0; CLcount < v_list4.Length(); CLcount++{ 
            ...
            v_local4 = ANY(try_4)
            ToList(OBJ(try_3)).PutAt(CLcount,v_local4)
            } 
          }
          } 
    ...
    { var g0063 *Language.CallMethod = Language.To_CallMethod(x)
      if ((g0063.Arg.Selector.Id() == C_nth.Id()) && 
          (g0063.Args.Length() == 2)) { 
        { var arg_5 *ClaireAny
          var try_7 EID
          try_7 = F_Generate_retreive_list_any(g0063.Args.At(0))
          if ErrorIn(try_7) {Result = try_7
          } else {
          arg_5 = ANY(try_7)
          { var arg_6 *ClaireAny
            var try_8 EID
            try_8 = F_Generate_retreive_list_any(g0063.Args.At(1))
        ...
        }  else if ((g0063.Arg.Selector.Id() == C_nth.Id()) && 
          (g0063.Args.Length() == 3)) { 
        { var arg_9 *ClaireAny
          var try_12 EID
          try_12 = F_Generate_retreive_list_any(g0063.Args.At(0))
          if ErrorIn(try_12) {Result = try_12
          } else {
          arg_9 = ANY(try_12)
          { var arg_10 *ClaireAny
            var try_13 EID
            try_13 = F_Generate_retreive_list_any(g0063.Args.At(1))
            if ErrorIn(try_13) {Result = try_13
            } else {
            arg_10 = ANY(try_13)
            { var arg_11 *ClaireAny
              var try_14 EID
              try_14 = F_Generate_retreive_list_any(g0063.Args.At(2))
        ...
        }  else if ((g0063.Arg.Selector.Id() == Core.C_Core_param_I.Id()) && 
          (g0063.Args.Length() == 2)) { 
        { var arg_15 *ClaireAny
          var try_17 EID
          try_17 = F_Generate_retreive_list_any(g0063.Args.At(0))
          if ErrorIn(try_17) {Result = try_17
          } else {
          arg_15 = ANY(try_17)
          { var arg_16 *ClaireAny
            var try_18 EID
            try_18 = F_Generate_retreive_list_any(g0063.Args.At(1))
        ...
        }  else if ((g0063.Arg.Selector.Id() == Core.C_U.Id()) && 
          (g0063.Args.Length() == 2)) { 
        { var arg_19 *ClaireAny
          var try_21 EID
          try_21 = F_Generate_retreive_list_any(g0063.Args.At(0))
          if ErrorIn(try_21) {Result = try_21
          } else {
          arg_19 = ANY(try_21)
          { var arg_20 *ClaireAny
            var try_22 EID
            try_22 = F_Generate_retreive_list_any(g0063.Args.At(1))
        ...
        }  else if ((g0063.Arg.Selector.Id() == C__dot_dot.Id()) && 
          (g0063.Args.Length() == 2)) { 
        { var arg_23 *ClaireAny
          var try_25 EID
          try_25 = F_Generate_retreive_list_any(g0063.Args.At(0))
          if ErrorIn(try_25) {Result = try_25
          } else {
          arg_23 = ANY(try_25)
          { var arg_24 *ClaireAny
            var try_26 EID
            try_26 = F_Generate_retreive_list_any(g0063.Args.At(1))
        ...
        Result = ToException(Core.C_general_error.Make(MakeString("we need to extend retreive_list to handle a type call: ~S").Id(),MakeConstantList(g0063.Id()).Id())).Close()
        } 
      } 
    } else {
...
func F_Generate_eid_prefix_class (s *ClaireClass) EID { 
  ...
  if ((s.Id() == Optimize.C_EID.Id()) || 
    ...
    PRINC("CHAR(")
    Result = EVOID
    }  else if (s.Id() == C_primitive.Id()) { 
    PRINC("ToPrimitive(ANY(")
    Result = EVOID
    }  else if (s.Id() == C_any.Id()) { 
    PRINC("ANY(")
    Result = EVOID
  ...
  if (g0065I == CTRUE) { 
    ...
    { 
      ...
      { 
        ...
        v_or3 = F_boolean_I_any(id_ask)
        if (v_or3 == CTRUE) {try_9 = EID{CTRUE.Id(),0}
        } else { 
          var try_10 EID
          try_10 = Optimize.F_Compile_identifiable_ask_any(a1)
          if ErrorIn(try_10) {try_9 = try_10
          } else {
          v_or3 = ToBoolean(OBJ(try_10))
          if (v_or3 == CTRUE) {try_9 = EID{CTRUE.Id(),0}
          } else { 
            var try_11 EID
      ...
      v_and2 = ToBoolean(OBJ(try_9))
      if (v_and2 == CFALSE) {try_8 = EID{CFALSE.Id(),0}
      } else { 
        var try_15 EID
...
func (c *GenerateGoProducer) CharExp_ask (x *ClaireAny) *ClaireBoolean { 
  ...
  if (C_char.Id() == x.Isa.Id()) { 
    ...
    { var g0070 *Language.CallMethod = Language.To_CallMethod(x)
      { var m *ClaireMethod = g0070.Arg
...
func F_Generate_go_signature_method (m *ClaireMethod) *ClaireList { 
  ...
  { var arg_1 *ClaireList
    { 
      ...
      v_list2 = m.Domain
      arg_1 = CreateList(ToType(C_class.Id()),v_list2.Length())
      for CLcount := 0; CLcount < v_list2.Length(); CLcount++{ 
        t = ToTypeExpression(v_list2.At(CLcount))
        v_local2 = t.Class_I().Id()
        arg_1.PutAt(CLcount,v_local2)
        } 
      } 
...
func F_Generate_full_signature_method (m *ClaireMethod) *ClaireList { 
  ...
  { var arg_1 *ClaireList
    { 
      ...
      var v_local2 *ClaireAny
      v_list2 = m.Domain
      arg_1 = CreateList(ToType(C_type.Id()),v_list2.Length())
      for CLcount := 0; CLcount < v_list2.Length(); CLcount++{ 
        t = ToTypeExpression(v_list2.At(CLcount))
        v_local2 = t.Id()
        arg_1.PutAt(CLcount,v_local2)
  ...
  if ((x.Isa.IsIn(Language.C_Call_method) == CTRUE) || 
    ...
    { var t1 *ClaireType
      ...
      if (Core.F_unique_ask_type(t1) == CTRUE) { 
        { var arg_4 *ClaireAny
          var try_5 EID
          try_5 = Core.F_the_type(t1)
          if ErrorIn(try_5) {Result = try_5
          } else {
          arg_4 = ANY(try_5)
          Result = EID{ToTypeExpression(arg_4).Class_I().Id(),0}
          }
          } 
        } else {
...
/* The go function for: g_expected(s:class) [status=0] */
func F_Generate_g_expected_class2 (s *ClaireClass) *ClaireClass { 
  if ((s.Id() == C_float.Id()) || 
      (s.Id() == C_integer.Id())) { 
    return  s
    } else {
...
// The EID go function for: g_expected @ class (throw: false) 
func E_Generate_g_expected_class2 (s EID) EID { 
  return EID{F_Generate_g_expected_class2(ToClass(OBJ(s)) ).Id(),0}} 
...
func F_Generate_g_clean_any (x *ClaireAny) EID { 
  ...
  { 
    ...
    v_and1 = ToBoolean(OBJ(try_1))
    if (v_and1 == CFALSE) {Result = EID{CFALSE.Id(),0}
    } else { 
      var try_2 EID
