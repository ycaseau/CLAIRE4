...
/* The go function for: expression!(p:go_producer,self:any,s:class) [status=1] */
func (p *GenerateGoProducer) Expression_I (self *ClaireAny,s *ClaireClass) EID { 
  var Result EID
  Result = Core.F_CALL(C_Generate_g_expression,ARGS(self.ToEID(),EID{s.Id(),0}))
  return Result} 

// The EID go function for: expression! @ go_producer (throw: true) 
func E_Generate_expression_I_go_producer (p EID,self EID,s EID) EID { 
  return ToGenerateGoProducer(OBJ(p)).Expression_I(ANY(self),ToClass(OBJ(s)) )} 

// made generic with print_true
/* The go function for: print_true(p:go_producer) [status=0] */
func (p *GenerateGoProducer) PrintTrue ()  { 
  PRINC("CTRUE")
  } 

// The EID go function for: print_true @ go_producer (throw: false) 
func E_Generate_print_true_go_producer (p EID) EID { 
  ToGenerateGoProducer(OBJ(p)).PrintTrue( )
  return EVOID} 

// generate an expressions with boundaries (parenthesis if necessary)
/* The go function for: bounded_expression(p:code_producer,self:any,s:class) [status=1] */
func (p *GenerateCodeProducer) BoundedExpression (self *ClaireAny,s *ClaireClass) EID { 
  var Result EID
  if (self.Isa.IsIn(Language.C_Assign) == CTRUE) { 
    { var g0077 *Language.Assign = Language.To_Assign(self)
      PRINC("(")
      Result = ToGenerateGoProducer(p.Id()).Expression_I(g0077.Id(),s)
      if !ErrorIn(Result) {
      PRINC(")")
      Result = EVOID
      }
    ...
    { var g0078 int = ToInteger(self).Value
      if (g0078 < 0) { 
        PRINC("(")
        Result = ToGenerateGoProducer(p.Id()).Expression_I(MakeInteger(g0078).Id(),s)
        if !ErrorIn(Result) {
        PRINC(")")
        Result = EVOID
        }
    ...
    { var g0079 float64 = ToFloat(self).Value
      if (g0079 < 0) { 
        PRINC("(")
        Result = ToGenerateGoProducer(p.Id()).Expression_I(MakeFloat(g0079).Id(),s)
        if !ErrorIn(Result) {
        PRINC(")")
        Result = EVOID
        }
    ...
    Result = ToGenerateGoProducer(p.Id()).Expression_I(self,s)
    } 
  return Result} 

// The EID go function for: bounded_expression @ code_producer (throw: true) 
func E_Generate_bounded_expression_code_producer (p EID,self EID,s EID) EID { 
  return ToGenerateCodeProducer(OBJ(p)).BoundedExpression(ANY(self),ToClass(OBJ(s)) )} 

// CLAIRE uses 1 .. n as array range, target languages use 0 .. n-1
/* The go function for: at_index(p:code_producer,x:any) [status=1] */
func (p *GenerateCodeProducer) AtIndex (x *ClaireAny) EID { 
  var Result EID
  if (C_integer.Id() == x.Isa.Id()) { 
    { var g0081 int = ToInteger(x).Value
      F_princ_integer((g0081-1))
      Result = EVOID
      } 
    } else {
    Result = ToGenerateGoProducer(p.Id()).Expression_I(x,C_integer)
    if !ErrorIn(Result) {
    PRINC("-1")
    Result = EVOID
    }
...
// The EID go function for: at_index @ code_producer (throw: true) 
func E_at_index_code_producer (p EID,x EID) EID { 
  return ToGenerateCodeProducer(OBJ(p)).AtIndex(ANY(x) )} 
...
func F_Generate_g_func_any (self *ClaireAny) EID { 
  ...
  if (self.Isa.IsIn(C_bag) == CTRUE) { 
    { var g0083 *ClaireBag = ToBag(self)
      { var arg_1 *ClaireAny
        var try_2 EID
        { 
          var x *ClaireAny
          ...
          var try_3 EID
          try_3 = Core.F_enumerate_any(g0083.Id())
    ...
    { var g0084 *Language.Construct = Language.To_Construct(self)
      if (((g0084.Isa.IsIn(Language.C_Set) == CTRUE) || 
            (g0084.Isa.IsIn(Language.C_List) == CTRUE)) || 
          (g0084.Isa.IsIn(Language.C_Tuple) == CTRUE)) { 
        { 
          var v_and4 *ClaireBoolean
          
          v_and4 = Core.F__inf_integer(g0084.Args.Length(),15)
    ...
    { var g0085 *Language.If = Language.To_If(self)
      { 
        var v_and3 *ClaireBoolean
        
        var try_15 EID
        try_15 = F_Generate_g_func_any(g0085.Test)
        if ErrorIn(try_15) {Result = try_15
        } else {
        v_and3 = ToBoolean(OBJ(try_15))
        if (v_and3 == CFALSE) {Result = EID{CFALSE.Id(),0}
        } else { 
          v_and3 = F_Generate_constant_ask_any(g0085.Arg)
          if (v_and3 == CFALSE) {Result = EID{CFALSE.Id(),0}
          } else { 
            v_and3 = F_Generate_constant_ask_any(g0085.Other)
            if (v_and3 == CFALSE) {Result = EID{CFALSE.Id(),0}
            } else { 
              Result = EID{CTRUE.Id(),0}} 
            } 
    ...
    { var g0086 *Language.And = Language.To_And(self)
      Result = F_Generate_g_func_any(g0086.Args.Id())
      } 
    }  else if (self.Isa.IsIn(Language.C_Or) == CTRUE) { 
    { var g0087 *Language.Or = Language.To_Or(self)
      Result = F_Generate_g_func_any(g0087.Args.Id())
      } 
    }  else if (self.Isa.IsIn(Language.C_Call) == CTRUE) { 
    { var g0088 *Language.Call = Language.To_Call(self)
      { 
        var v_and3 *ClaireBoolean
        
        var try_16 EID
        try_16 = F_Generate_g_func_any(g0088.Args.Id())
        if ErrorIn(try_16) {Result = try_16
        } else {
        v_and3 = ToBoolean(OBJ(try_16))
        if (v_and3 == CFALSE) {Result = EID{CFALSE.Id(),0}
        } else { 
          v_and3 = Core.F__I_equal_any(g0088.Selector.Id(),Optimize.C_Compile_object_I.Id())
          if (v_and3 == CFALSE) {Result = EID{CFALSE.Id(),0}
          } else { 
            var try_17 EID
    ...
    { var g0089 *Language.Super = Language.To_Super(self)
      Result = F_Generate_g_func_any(g0089.Args.Id())
      } 
    }  else if (self.Isa.IsIn(Language.C_Call_method) == CTRUE) { 
    { var g0090 *Language.CallMethod = Language.To_CallMethod(self)
      { 
        var v_and3 *ClaireBoolean
        
        var try_26 EID
        try_26 = F_Generate_g_func_any(g0090.Args.Id())
        if ErrorIn(try_26) {Result = try_26
        } else {
        v_and3 = ToBoolean(OBJ(try_26))
        if (v_and3 == CFALSE) {Result = EID{CFALSE.Id(),0}
        } else { 
          var try_27 EID
          { 
            ...
            v_or5 = Equal(g0090.Arg.Id(),Optimize.C_Compile_m_unsafe.Value)
            if (v_or5 == CTRUE) {try_27 = EID{CTRUE.Id(),0}
            } else { 
              var try_28 EID
    ...
    { var g0091 *Language.CallSlot = Language.To_CallSlot(self)
      Result = F_Generate_g_func_any(g0091.Arg)
      } 
    }  else if (self.Isa.IsIn(Language.C_Call_table) == CTRUE) { 
    { var g0092 *Language.CallTable = Language.To_CallTable(self)
      Result = F_Generate_g_func_any(g0092.Arg)
      } 
    }  else if (self.Isa.IsIn(Language.C_Call_array) == CTRUE) { 
    { var g0093 *Language.CallArray = Language.To_CallArray(self)
      Result = F_Generate_g_func_any(g0093.Arg)
      } 
    }  else if (self.Isa.IsIn(Language.C_Cast) == CTRUE) { 
    { var g0094 *Language.Cast = Language.To_Cast(self)
      Result = F_Generate_g_func_any(g0094.Arg)
      } 
    }  else if (self.Isa.IsIn(Optimize.C_Compile_C_cast) == CTRUE) { 
    { var g0095 *Optimize.Compile_CCast = Optimize.To_Compile_CCast(self)
      Result = F_Generate_g_func_any(g0095.Arg)
      } 
    } else {
      ...
      { var n int = self.Args.Length()
        if (p.Id() == Core.C_mClaire_get_stack.Id()) { 
          ...
          { var y int = Optimize.C_compiler.Safety
            ...
            Result = Core.F_CALL(C_Generate_g_expression,ARGS(self.Args.At(0).ToEID(),EID{s.Id(),0}))
            if !ErrorIn(Result) {
            { 
          ...
          if !ErrorIn(Result) {
          PRINC(".Included(")
          Result = Core.F_CALL(C_Generate_g_expression,ARGS(self.Args.At(1).ToEID(),EID{C_type.Id(),0}))
          if !ErrorIn(Result) {
          PRINC(")")
          F_Generate_object_post_class(C_boolean,s)
          Result = EVOID
          ...
          if !ErrorIn(Result) {
          PRINC("]=")
          Result = Core.F_CALL(C_Generate_g_expression,ARGS(self.Args.At(1).ToEID(),EID{Optimize.C_EID.Id(),0}))
          }
          ...
          if !ErrorIn(Result) {
          PRINC(".AddSlot(")
          Result = Core.F_CALL(C_Generate_g_expression,ARGS(self.Args.At(1).ToEID(),EID{C_property.Id(),0}))
          if !ErrorIn(Result) {
          PRINC(",")
          Result = Core.F_CALL(C_Generate_g_expression,ARGS(self.Args.At(2).ToEID(),EID{C_type.Id(),0}))
          if !ErrorIn(Result) {
          PRINC(",")
          Result = Core.F_CALL(C_Generate_g_expression,ARGS(self.Args.At(3).ToEID(),EID{C_any.Id(),0}))
          if !ErrorIn(Result) {
          PRINC(")")
      ...
      { var a1 *ClaireAny
        ...
        { var dm *ClaireClass = Core.F_domain_I_restriction(ToRestriction(m.Id()))
          if ((p.Id() == C__dash.Id()) && 
            ...
            if !ErrorIn(Result) {
            PRINC("(-")
            Result = c.BoundedExpression(a1,s)
            if !ErrorIn(Result) {
            PRINC(")")
            F_Generate_cast_post_class(dm,s)
            Result = EVOID
            }}
            }  else if ((m.Id() == Optimize.C_Compile__starexp2_integer_star.Value) && 
              (s.Id() == C_integer.Id())) { 
            PRINC("(1 << ")
            Result = Core.F_CALL(C_Generate_g_expression,ARGS(a1.ToEID(),EID{C_integer.Id(),0}))
            if !ErrorIn(Result) {
            ...
            { 
              ...
              v_and6 = Equal(p.Id(),Core.C_owner.Id())
            ...
            if (g0103I == CTRUE) { 
              ...
              { 
                ...
                v_and7 = Equal(m.Id(),C_Generate__starlength_bag_star.Value)
              ...
              if (g0104I == CTRUE) { 
                ...
                { 
                  ...
                  v_and8 = Equal(p.Id(),C_integer_I.Id())
                  if (v_and8 == CFALSE) {try_6 = EID{CFALSE.Id(),0}
                  } else { 
                    v_and8 = Equal(Core.F_domain_I_restriction(ToRestriction(m.Id())).Id(),C_char.Id())
                    if (v_and8 == CFALSE) {try_6 = EID{CFALSE.Id(),0}
                    } else { 
                      var try_7 EID
                ...
                if (g0105I == CTRUE) { 
                  ...
                  { 
                    ...
                    v_and9 = Equal(m.Id(),C_Generate__starnot_star.Value)
                    if (v_and9 == CFALSE) {try_11 = EID{CFALSE.Id(),0}
                    } else { 
                      var try_12 EID
          ...
          { var s1 *ClaireClass
            ...
            if ((p.Id() == C_class_I.Id()) && 
              ...
              { 
                ...
                v_and7 = Equal(m.Domain.ValuesO()[0],m.Domain.ValuesO()[1])
                if (v_and7 == CFALSE) {g0111I = CFALSE
                } else { 
                  v_and7 = MakeBoolean((s1.Id() == C_integer.Id()) || (s1.Id() == C_float.Id()))
                  if (v_and7 == CFALSE) {g0111I = CFALSE
                  } else { 
                    { 
                      var v_or10 *ClaireBoolean
                      
                      v_or10 = c.OpenOperators.Memq(p.Id())
                      if (v_or10 == CTRUE) {v_and7 = CTRUE
                      } else { 
                        { 
                          var v_and12 *ClaireBoolean
                          
                          v_and12 = c.DivOperators.Memq(p.Id())
                          if (v_and12 == CFALSE) {v_or10 = CFALSE
                          } else { 
                            if (C_integer.Id() == a2.Isa.Id()) { 
                              { var g0108 int = ToInteger(a2).Value
                                v_and12 = Core.F__I_equal_any(MakeInteger(g0108).Id(),MakeInteger(0).Id())
                                } 
                              }  else if (C_float.Id() == a2.Isa.Id()) { 
                              { var g0109 float64 = ToFloat(a2).Value
                                v_and12 = Core.F__I_equal_any(MakeFloat(g0109).Id(),MakeFloat(0).Id())
                                } 
                              } else {
                              v_and12 = F__sup_equal_integer(Optimize.C_compiler.Safety,3)
                              } 
                            if (v_and12 == CFALSE) {v_or10 = CFALSE
                            } else { 
                              v_or10 = CTRUE} 
              ...
              if (g0111I == CTRUE) { 
                ...
                if !ErrorIn(Result) {
                PRINC("(")
                Result = c.BoundedExpression(a1,s1)
                if !ErrorIn(Result) {
                ...
                Result = c.BoundedExpression(a2,s1)
                if !ErrorIn(Result) {
                PRINC(")")
                ...
                { 
                  ...
                  v_and8 = Equal(m.Id(),C_Generate__starcontain_list_star.Value)
                ...
                if (g0112I == CTRUE) { 
                  ...
                  Result = Core.F_CALL(C_Generate_belong_exp,ARGS(EID{c.Id(),0},
                    a1.ToEID(),
                    a2.ToEID(),
                    EID{s.Id(),0}))
                  ...
                  { 
                    ...
                    { 
                      ...
                      { 
                        ...
                        { 
                          ...
                          v_and12 = MakeBoolean((m.Id() == C_Generate__starnth_list_star.Value) || (m.Id() == C_Generate__starnth_tuple_star.Value))
                          if (v_and12 == CFALSE) {try_14 = EID{CFALSE.Id(),0}
                            ...
                            { 
                              ...
                              v_or14 = F__sup_equal_integer(Optimize.C_compiler.Safety,5)
                              if (v_or14 == CTRUE) {try_15 = EID{CTRUE.Id(),0}
                              } else { 
                                var try_16 EID
                                { 
                                  var v_and16 *ClaireBoolean
                                  
                                  v_and16 = F__sup_equal_integer(Optimize.C_compiler.Safety,3)
                                  if (v_and16 == CFALSE) {try_16 = EID{CFALSE.Id(),0}
                                  } else { 
                                    var try_17 EID
                                    { var arg_18 *ClaireBoolean
                                      var try_19 EID
                                      try_19 = Optimize.F_Compile_g_throw_any(a2)
                                      if ErrorIn(try_19) {try_17 = try_19
                                      } else {
                                      arg_18 = ToBoolean(OBJ(try_19))
                                      try_17 = EID{arg_18.Not.Id(),0}
                                      }
                                      } 
                                    if ErrorIn(try_17) {try_16 = try_17
                                    } else {
                                    v_and16 = ToBoolean(OBJ(try_17))
                                    if (v_and16 == CFALSE) {try_16 = EID{CFALSE.Id(),0}
                                    } else { 
                        ...
                        v_or11 = ToBoolean(OBJ(try_14))
                        if (v_or11 == CTRUE) {try_13 = EID{CTRUE.Id(),0}
                        } else { 
                          v_or11 = Equal(m.Id(),C_Generate__starnth_1_list_star.Value)
                          if (v_or11 == CTRUE) {try_13 = EID{CTRUE.Id(),0}
                          } else { 
                            v_or11 = Equal(m.Id(),C_Generate__starnth_1_tuple_star.Value)
                            if (v_or11 == CTRUE) {try_13 = EID{CTRUE.Id(),0}
                            } else { 
                              v_or11 = Equal(m.Id(),C_Generate__starnth_1_array_star.Value)
                              if (v_or11 == CTRUE) {try_13 = EID{CTRUE.Id(),0}
                              } else { 
                      ...
                      if (v_and10 == CFALSE) {try_12 = EID{CFALSE.Id(),0}
                        ...
                        var try_20 EID
                        { var arg_21 *ClaireClass
                          var try_22 EID
                          try_22 = F_Generate_g_member_any(a1)
                          if ErrorIn(try_22) {try_20 = try_22
                          } else {
                          arg_21 = ToClass(OBJ(try_22))
                          try_20 = EID{Core.F__I_equal_any(arg_21.Id(),C_any.Id()).Id(),0}
                          }
                          } 
                        if ErrorIn(try_20) {try_12 = try_20
                        } else {
                        v_and10 = ToBoolean(OBJ(try_20))
                        if (v_and10 == CFALSE) {try_12 = EID{CFALSE.Id(),0}
                        } else { 
                    ...
                    v_or9 = ToBoolean(OBJ(try_12))
                    if (v_or9 == CTRUE) {try_11 = EID{CTRUE.Id(),0}
                    } else { 
                  ...
                  g0113I = ToBoolean(OBJ(try_11))
                  if (g0113I == CTRUE) { 
                    { var s1 *ClaireClass
                      var try_23 EID
                      if (m.Selector.Id() == Core.C_mClaire_nth_object.Id()) { 
                        try_23 = EID{C_object.Id(),0}
                        } else {
                        { var arg_24 *ClaireClass
                          var try_25 EID
                          try_25 = F_Generate_g_member_any(a1)
                          if ErrorIn(try_25) {try_23 = try_25
                          } else {
                          arg_24 = ToClass(OBJ(try_25))
                          try_23 = EID{F_Generate_type_sort_type(ToType(arg_24.Id())).Id(),0}
                          }
                          } 
                        } 
                      if ErrorIn(try_23) {Result = try_23
                      } else {
                      s1 = ToClass(OBJ(try_23))
                      Result = F_Generate_cast_prefix_class(s1,s)
                      if !ErrorIn(Result) {
                      Result = Core.F_CALL(C_Generate_g_expression,ARGS(a1.ToEID(),EID{C_list.Id(),0}))
                      if !ErrorIn(Result) {
                      PRINC(".")
                      { var arg_26 *ClaireClass
                        var try_27 EID
                        try_27 = F_Generate_g_member_any(a1)
                        if ErrorIn(try_27) {Result = try_27
                        } else {
                        arg_26 = ToClass(OBJ(try_27))
                        F_Generate_valuesSlot_class(arg_26)
                        Result = EVOID
                        }
                        } 
                      if !ErrorIn(Result) {
                    ...
                    { var sbag *ClaireClass
                      ...
                      { var _Ztype *ClaireClass
                        var try_28 EID
                        try_28 = F_Generate_g_member_any(a1)
                        if ErrorIn(try_28) {Result = try_28
                        } else {
                        _Ztype = ToClass(OBJ(try_28))
                    ...
                    { 
                      ...
                      v_and10 = Equal(C_table.Id(),a1.Isa.Id())
                      if (v_and10 == CFALSE) {try_29 = EID{CFALSE.Id(),0}
                      } else { 
                        v_and10 = Equal(p.Id(),C_nth.Id())
                        if (v_and10 == CFALSE) {try_29 = EID{CFALSE.Id(),0}
                        } else { 
                          var try_30 EID
                          { 
                            ...
                            { var arg_32 *ClaireType
                              var try_33 EID
                              try_33 = Core.F_CALL(Optimize.C_c_type,ARGS(a2.ToEID()))
                              if ErrorIn(try_33) {try_31 = try_33
                              } else {
                              arg_32 = ToType(OBJ(try_33))
                              try_31 = EID{arg_32.Included(ToRelation(a1).Domain).Id(),0}
                              }
                              } 
                            if ErrorIn(try_31) {try_30 = try_31
                            } else {
                            v_or13 = ToBoolean(OBJ(try_31))
                            if (v_or13 == CTRUE) {try_30 = EID{CTRUE.Id(),0}
                            } else { 
                              v_or13 = F__sup_equal_integer(Optimize.C_compiler.Safety,2)
                              if (v_or13 == CTRUE) {try_30 = EID{CTRUE.Id(),0}
                              } else { 
                                try_30 = EID{CFALSE.Id(),0}} 
                              } 
                          ...
                          if ErrorIn(try_30) {try_29 = try_30
                          } else {
                          v_and10 = ToBoolean(OBJ(try_30))
                          if (v_and10 == CFALSE) {try_29 = EID{CFALSE.Id(),0}
                          } else { 
                            try_29 = EID{CTRUE.Id(),0}} 
                          } 
                        } 
                      }
                      } 
                    ...
                    if (g0114I == CTRUE) { 
                      ...
                      if !ErrorIn(Result) {
                      PRINC("MakeBoolean(")
                      Result = Core.F_CALL(C_Generate_b_expression,ARGS(EID{c.Id(),0},EID{self.Id(),0},EID{CTRUE.Id(),0}))
                      if !ErrorIn(Result) {
                      PRINC(")")
...
func F_Generate_inline_exp_go_producer4 (c *GenerateGoProducer,self *Language.CallMethod,s *ClaireClass) EID { 
  ...
  { var m *ClaireMethod = self.Arg
    { var a1 *ClaireAny = self.Args.At(0)
      { var a2 *ClaireAny = self.Args.At(1)
        { var a3 *ClaireAny = self.Args.At(2)
          var g0115I *ClaireBoolean
          var try_1 EID
          { 
            var v_and5 *ClaireBoolean
            
            v_and5 = Equal(m.Id(),C_Generate__starnth_equal_list_star.Value)
            if (v_and5 == CFALSE) {try_1 = EID{CFALSE.Id(),0}
            } else { 
              v_and5 = F__sup_equal_integer(Optimize.C_compiler.Safety,3)
              if (v_and5 == CFALSE) {try_1 = EID{CFALSE.Id(),0}
              } else { 
                var try_2 EID
          ...
          if (g0115I == CTRUE) { 
            ...
            { var arg_7 *ClaireClass
              ...
              { var arg_9 *ClaireClass
                var try_10 EID
                try_10 = F_Generate_g_member_any(a1)
                if ErrorIn(try_10) {try_8 = try_10
                } else {
                arg_9 = ToClass(OBJ(try_10))
                try_8 = EID{F_Generate_g_expected_class2(arg_9).Id(),0}
                }
                } 
              if ErrorIn(try_8) {Result = try_8
              } else {
            ...
            if (a1.Isa.IsIn(C_property) == CTRUE) { 
              { var m *ClaireAny = self.Args.At(5)
                F_Generate_preCore_ask_void()
                ...
                { var arg_11 int
                  ...
                  var try_13 EID
                  try_13 = Optimize.F_Compile_can_throw_status_method(ToMethod(m))
                  if ErrorIn(try_13) {try_12 = try_13
                  } else {
                  g0116I = ToBoolean(OBJ(try_13))
                  if (g0116I == CTRUE) { 
                    try_12 = EID{C__INT,IVAL(1)}
                    } else {
                    try_12 = EID{C__INT,IVAL(0)}
                    } 
                  }
                  if ErrorIn(try_12) {Result = try_12
                  } else {
                  arg_11 = INT(try_12)
                  F_princ_integer(arg_11)
                  Result = EVOID
                  }
                  } 
                if !ErrorIn(Result) {
              ...
              if !ErrorIn(Result) {
              PRINC(",")
              Result = Core.F_CALL(C_Generate_g_expression,ARGS(self.Args.At(3).ToEID(),EID{C_integer.Id(),0}))
              if !ErrorIn(Result) {
              PRINC(",")
              Result = Core.F_CALL(C_Generate_g_expression,ARGS(self.Args.At(4).ToEID(),EID{C_function.Id(),0}))
              if !ErrorIn(Result) {
              PRINC(")")
              Result = EVOID
              }}}}}
        ...
        { var _Zsig *ClaireList = F_Generate_go_signature_method(m)
          { var sm *ClaireClass
            ...
            if (F_Generate_goMethod_ask_any(m.Id()) == CTRUE) { 
              Result = F_Generate_external_casted_arg_any(l.At(0),ToClass(_Zsig.ValuesO()[0]),0,Core.F__sup_integer(l.Length(),4))
              if !ErrorIn(Result) {
              PRINC(".")
              F_Generate_goMethod_method(m)
              PRINC("(")
              { var n int = 2
                { var g0119 int = l.Length()
                  Result= EID{CFALSE.Id(),0}
                  for (n <= g0119) { 
                    var loop_3 EID
                    _ = loop_3
        ...
        { var g0124 int = n
          ...
          for (i <= g0124) { 
            ...
            { var x *ClaireAny = self.Args.At(i-1)
              loop_1 = Core.F_CALL(C_Generate_b_expression,ARGS(EID{Optimize.C_PRODUCER.Value,0},x.ToEID(),EID{CTRUE.Id(),0}))
              if ErrorIn(loop_1) {Result = loop_1
              break
              } else {
              if (i < n) { 
...
func F_Generate_g_expression_C_cast (self *Optimize.Compile_CCast,s *ClaireClass) EID { 
  var Result EID
  if (self.Isa.IsIn(Optimize.C_Compile_Super_cast) == CTRUE) { 
    PRINC("/*Super_cast*/")
    F_Generate_cast_class_class(self.SetArg)
    PRINC("(")
  ...
  { var p *ClaireAny = a.Params
    if (C_integer.Id() == p.Isa.Id()) { 
      ...
      { var g0127 *ClaireList = ToList(p)
        ...
        if !ErrorIn(Result) {
        Result = Core.F_CALL(C_Generate_g_expression,ARGS(ToList(OBJ(Core.F_CALL(C_args,ARGS(l.ToEID())))).At(0).ToEID(),EID{C_integer.Id(),0}))
        if !ErrorIn(Result) {
        PRINC(" * ")
        Result = Core.F_CALL(C_princ,ARGS(g0127.At(0).ToEID()))
        if !ErrorIn(Result) {
        PRINC(" + ")
        Result = Core.F_CALL(C_Generate_g_expression,ARGS(ToList(OBJ(Core.F_CALL(C_args,ARGS(l.ToEID())))).At(1).ToEID(),EID{C_integer.Id(),0}))
        if !ErrorIn(Result) {
        PRINC(" - ")
        Result = Core.F_CALL(C_princ,ARGS(g0127.At(1).ToEID()))
        }}}
        }
        } 
      } else {
  ...
  { var sa *ClaireClass
    ...
    { var sm *ClaireClass
      ...
      if (sm.Id() != C_any.Id()) { 
        ...
        PRINC("[")
        Result = ToGenerateCodeProducer(Optimize.C_PRODUCER.Value).AtIndex(self.Arg)
        if !ErrorIn(Result) {
        PRINC("]")
        Result = EVOID
        }}
...
/* The go function for: b_expression(p:code_producer,self:any,pos?:boolean) [status=1] */
func F_Generate_b_expression_code_producer1 (p *GenerateCodeProducer,self *ClaireAny,pos_ask *ClaireBoolean) EID { 
  var Result EID
  if (self == CTRUE.Id()) { 
    PRINC("true")
    Result = EVOID
    ...
    PRINC("(")
    Result = ToGenerateGoProducer(p.Id()).Expression_I(self,C_boolean)
    if !ErrorIn(Result) {
    PRINC(" ")
    F_Generate_sign_equal_boolean(pos_ask)
    PRINC(" ")
    Result = Core.F_CALL(C_Generate_print_true,ARGS(EID{p.Id(),0}))
    if !ErrorIn(Result) {
    PRINC(")")
    Result = EVOID
    }}
...
// The EID go function for: b_expression @ list<type_expression>(code_producer, any, boolean) (throw: true) 
func E_Generate_b_expression_code_producer1 (p EID,self EID,pos_ask EID) EID { 
  return F_Generate_b_expression_code_producer1(ToGenerateCodeProducer(OBJ(p)),ANY(self),ToBoolean(OBJ(pos_ask)) )} 

// strange : not clear why we should see a C_cast here
/* The go function for: b_expression(p:code_producer,self:Compile/C_cast,pos?:boolean) [status=1] */
func F_Generate_b_expression_code_producer2 (p *GenerateCodeProducer,self *Optimize.Compile_CCast,pos_ask *ClaireBoolean) EID { 
  var Result EID
  Result = Core.F_CALL(C_Generate_b_expression,ARGS(EID{p.Id(),0},self.Arg.ToEID(),EID{pos_ask.Id(),0}))
  return Result} 

// The EID go function for: b_expression @ list<type_expression>(code_producer, Compile/C_cast, boolean) (throw: true) 
func E_Generate_b_expression_code_producer2 (p EID,self EID,pos_ask EID) EID { 
  return F_Generate_b_expression_code_producer2(ToGenerateCodeProducer(OBJ(p)),Optimize.To_Compile_CCast(OBJ(self)),ToBoolean(OBJ(pos_ask)) )} 

// If is supported with IfThenElse (means that all terms will be evaluated) [generic]
/* The go function for: b_expression(p:code_producer,self:If,pos?:boolean) [status=1] */
func F_Generate_b_expression_code_producer3 (p *GenerateCodeProducer,self *Language.If,pos_ask *ClaireBoolean) EID { 
  var Result EID
  if (F_boolean_I_any(self.Other) == CTRUE) { 
    PRINC("(")
    Result = Core.F_CALL(C_Generate_b_expression,ARGS(EID{p.Id(),0},self.Test.ToEID(),EID{CTRUE.Id(),0}))
    if !ErrorIn(Result) {
    PRINC(" ? ")
    Result = Core.F_CALL(C_Generate_b_expression,ARGS(EID{p.Id(),0},self.Arg.ToEID(),EID{pos_ask.Id(),0}))
    if !ErrorIn(Result) {
    PRINC(" : ")
    Result = Core.F_CALL(C_Generate_b_expression,ARGS(EID{p.Id(),0},self.Other.ToEID(),EID{pos_ask.Id(),0}))
    if !ErrorIn(Result) {
    PRINC(")")
    Result = EVOID
    }}}
    ...
    PRINC("(")
    Result = Core.F_CALL(C_Generate_b_expression,ARGS(EID{p.Id(),0},self.Test.ToEID(),EID{pos_ask.Id(),0}))
    if !ErrorIn(Result) {
    PRINC(" ")
...
// The EID go function for: b_expression @ list<type_expression>(code_producer, If, boolean) (throw: true) 
func E_Generate_b_expression_code_producer3 (p EID,self EID,pos_ask EID) EID { 
  return F_Generate_b_expression_code_producer3(ToGenerateCodeProducer(OBJ(p)),Language.To_If(OBJ(self)),ToBoolean(OBJ(pos_ask)) )} 

// for a AND, we can used the && C operation [generic]
/* The go function for: b_expression(p:code_producer,self:And,pos?:boolean) [status=1] */
func F_Generate_b_expression_code_producer4 (p *GenerateCodeProducer,self *Language.And,pos_ask *ClaireBoolean) EID { 
  var Result EID
  { var l *ClaireList = self.Args
    { var m int = l.Length()
      { var n int = 0
        { var _Zl int = Optimize.C_OPT.Level
          ...
          { 
            ...
            for i_it := 0; i_it < x_len; i_it++ { 
              ...
              n = (n+1)
              if (n == m) { 
                loop_1 = Core.F_CALL(C_Generate_b_expression,ARGS(EID{p.Id(),0},x.ToEID(),EID{pos_ask.Id(),0}))
                } else {
                PRINC("(")
                loop_1 = Core.F_CALL(C_Generate_b_expression,ARGS(EID{p.Id(),0},x.ToEID(),EID{pos_ask.Id(),0}))
                if ErrorIn(loop_1) {Result = loop_1
...
// The EID go function for: b_expression @ list<type_expression>(code_producer, And, boolean) (throw: true) 
func E_Generate_b_expression_code_producer4 (p EID,self EID,pos_ask EID) EID { 
  return F_Generate_b_expression_code_producer4(ToGenerateCodeProducer(OBJ(p)),Language.To_And(OBJ(self)),ToBoolean(OBJ(pos_ask)) )} 

// idem for OR: we use ||  [generic]
/* The go function for: b_expression(p:code_producer,self:Or,pos?:boolean) [status=1] */
func F_Generate_b_expression_code_producer5 (p *GenerateCodeProducer,self *Language.Or,pos_ask *ClaireBoolean) EID { 
  var Result EID
  { var l *ClaireList = self.Args
    { var m int = l.Length()
      { var n int = 0
...
// The EID go function for: b_expression @ list<type_expression>(code_producer, Or, boolean) (throw: true) 
...
func F_Generate_b_expression_code_producer6 (c *GenerateCodeProducer,self *Language.Call,pos_ask *ClaireBoolean) EID { 
  ...
  { var p *ClaireProperty = self.Selector
    if (p.Id() == C__Z.Id()) { 
      PRINC("(")
      Result = Core.F_CALL(C_Generate_belong_exp,ARGS(EID{c.Id(),0},
        self.Args.At(0).ToEID(),
        self.Args.At(1).ToEID(),
        EID{C_boolean.Id(),0}))
      if !ErrorIn(Result) {
      PRINC(" ")
      F_Generate_sign_equal_boolean(pos_ask)
      PRINC(" ")
      Result = Core.F_CALL(C_Generate_print_true,ARGS(EID{c.Id(),0}))
      if !ErrorIn(Result) {
      PRINC(")")
      Result = EVOID
      }}
      ...
      PRINC("(")
      Result = ToGenerateGoProducer(c.Id()).Expression_I(self.Id(),C_boolean)
      if !ErrorIn(Result) {
      PRINC(" ")
      F_Generate_sign_equal_boolean(pos_ask)
      PRINC(" ")
...
// The EID go function for: b_expression @ list<type_expression>(code_producer, Call, boolean) (throw: true) 
func E_Generate_b_expression_code_producer6 (c EID,self EID,pos_ask EID) EID { 
  return F_Generate_b_expression_code_producer6(ToGenerateCodeProducer(OBJ(c)),Language.To_Call(OBJ(self)),ToBoolean(OBJ(pos_ask)) )} 

// generic !
/* The go function for: b_expression(p:code_producer,self:Call_method1,pos?:boolean) [status=1] */
func F_Generate_b_expression_code_producer7 (p *GenerateCodeProducer,self *Language.CallMethod1,pos_ask *ClaireBoolean) EID { 
  var Result EID
  { var m *ClaireMethod = self.Arg
    { var a1 *ClaireAny = self.Args.At(0)
      if (m.Id() == C_Generate__starnot_star.Value) { 
        Result = Core.F_CALL(C_Generate_b_expression,ARGS(EID{p.Id(),0},a1.ToEID(),EID{pos_ask.Not.Id(),0}))
        }  else if (m.Id() == C_Generate__starknown_star.Value) { 
        Result = ToGenerateGoProducer(p.Id()).EqualExp(a1,
          pos_ask.Not,
          CNULL,
          CTRUE.Id())
        ...
        PRINC("(")
        Result = ToGenerateGoProducer(p.Id()).Expression_I(self.Id(),C_boolean)
        if !ErrorIn(Result) {
        PRINC(" ")
        F_Generate_sign_equal_boolean(pos_ask)
        PRINC(" ")
...
// The EID go function for: b_expression @ list<type_expression>(code_producer, Call_method1, boolean) (throw: true) 
func E_Generate_b_expression_code_producer7 (p EID,self EID,pos_ask EID) EID { 
  return F_Generate_b_expression_code_producer7(ToGenerateCodeProducer(OBJ(p)),Language.To_CallMethod1(OBJ(self)),ToBoolean(OBJ(pos_ask)) )} 

// same thing for two arguments functions => generic
// equal_exp is in gogen.cl
/* The go function for: b_expression(c:code_producer,self:Call_method2,pos?:boolean) [status=1] */
func F_Generate_b_expression_code_producer8 (c *GenerateCodeProducer,self *Language.CallMethod2,pos_ask *ClaireBoolean) EID { 
  var Result EID
  { var m *ClaireMethod = self.Arg
    { var p *ClaireProperty = m.Selector
      { var lop *ClaireList = c.OpenComparators
        { var a1 *ClaireAny = self.Args.At(0)
          { var a2 *ClaireAny = self.Args.At(1)
            if (p.Id() == Core.C__I_equal.Id()) { 
              Result = ToGenerateGoProducer(c.Id()).EqualExp(a1,
                pos_ask.Not,
                a2,
                CFALSE.Id())
              ...
              PRINC("(")
              Result = ToGenerateGoProducer(c.Id()).Expression_I(a1,Core.F_domain_I_restriction(ToRestriction(m.Id())))
              if !ErrorIn(Result) {
              PRINC(" ")
              if (pos_ask == CTRUE) { 
              ...
              if !ErrorIn(Result) {
              PRINC(" ")
              Result = ToGenerateGoProducer(c.Id()).Expression_I(a2,Core.F_domain_I_restriction(ToRestriction(m.Id())))
              if !ErrorIn(Result) {
              PRINC(")")
              Result = EVOID
              }}}
              ...
              Result = ToGenerateGoProducer(c.Id()).Expression_I(a1,C_integer)
              if !ErrorIn(Result) {
              PRINC(",")
              Result = ToGenerateGoProducer(c.Id()).Expression_I(a2,C_integer)
              if !ErrorIn(Result) {
              PRINC(") ")
              F_Generate_sign_equal_boolean(pos_ask)
              PRINC(" ")
              ...
              PRINC("(")
              Result = ToGenerateGoProducer(c.Id()).Expression_I(a1,C_class)
              if !ErrorIn(Result) {
              PRINC(".IsIn(")
              Result = ToGenerateGoProducer(c.Id()).Expression_I(a2,C_class)
              if !ErrorIn(Result) {
              PRINC(") ")
              F_Generate_sign_equal_boolean(pos_ask)
              PRINC(" ")
...
// The EID go function for: b_expression @ list<type_expression>(code_producer, Call_method2, boolean) (throw: true) 
func E_Generate_b_expression_code_producer8 (c EID,self EID,pos_ask EID) EID { 
  return F_Generate_b_expression_code_producer8(ToGenerateCodeProducer(OBJ(c)),Language.To_CallMethod2(OBJ(self)),ToBoolean(OBJ(pos_ask)) )} 

// belong_exp is not generic but is a producer method
// assumption : we require a boolean (s = boolean input constraint)
/* The go function for: belong_exp(p:go_producer,a1:any,a2:any,s:class) [status=1] */
func (p *GenerateGoProducer) BelongExp (a1 *ClaireAny,a2 *ClaireAny,s *ClaireClass) EID { 
  var Result EID
  ...
  if (g0134I == CTRUE) { 
    ...
    { 
      ...
      if (v_and2 == CFALSE) {try_4 = EID{CFALSE.Id(),0}
      } else { 
        var try_8 EID
        { var arg_9 *ClaireClass
          var try_10 EID
    ...
    if (g0135I == CTRUE) { 
      ...
      { 
        ...
        v_or3 = ToBoolean(OBJ(try_12))
        if (v_or3 == CTRUE) {try_11 = EID{CTRUE.Id(),0}
        } else { 
          var try_15 EID
...
// The EID go function for: belong_exp @ go_producer (throw: true) 
func E_Generate_belong_exp_go_producer (p EID,a1 EID,a2 EID,s EID) EID { 
  return ToGenerateGoProducer(OBJ(p)).BelongExp(ANY(a1),
    ANY(a2),
    ToClass(OBJ(s)) )} 
