                    ...
                    { 
                        ...
                        if ((ToString(l.ValuesO()[1-1]).Value == MakeString("?").Value) || 
                          ...
                          if (l.Length() >= 2) { 
                            ...
                            { 
                              ...
                              try_4 = Core.F_new_class2(Core.C_global_variable,Core.F_symbol_I_string2(ToString(l.ValuesO()[2-1])))
                              /* ERROR PROTECTION INSERTED (va_arg1-loop_1) */
                              if ErrorIn(try_4) {loop_1 = try_4
                              } else {
                              va_arg1 = Core.ToGlobalVariable(OBJ(try_4))
                              va_arg2 = CTRUE.Id()
                          ...
                          if (l.Length() >= 2) { 
                            ...
                            loop_1 = ToException(Core.C_general_error.Make(MakeString("option: -cx <filename>").Id(),CNIL.Id())).Close()
                            } 
                      ...
                      if (_Zout.Value == MakeString("").Value) { 
                        if (_Zcm.Value != MakeString("").Value) { 
                          _Zout = _Zcm
                          }  else if (_Zcx.Value != MakeString("").Value) { 
                          _Zout = _Zcx
                          } 
                        } 
                      ...
                      /*integer->integer*/if (_Zcx.Value != MakeString("").Value) { 
                        /*g_try(v2:"Result",loop:false) */
                        ...
                        { var arg_5 *ClaireModule  
                          _ = arg_5
                          var try_6 EID 
                          /*g_try(v2:"try_6",loop:false) */
                          try_6 = F_Generate_string2module_string(_Zcx)
                          /* ERROR PROTECTION INSERTED (arg_5-Result) */
                          if ErrorIn(try_6) {Result = try_6
                          } else {
                          arg_5 = ToModule(OBJ(try_6))
                          Result = F_system_file_module(arg_5,_Zout)
                          }
                          } 
                        /* ERROR PROTECTION INSERTED (Result-Result) */
                        if !ErrorIn(Result) {
...
//   - the main function
/* {1} The go function for: system_file(m:module,%out:string) [status=1] */
...
func E_system_file_module (m EID,_Zout EID) EID { 
    return F_system_file_module(ToModule(OBJ(m)),ToString(OBJ(_Zout)) )} 
...
/* {1} The go function for: main_function(l_used:list[module]) [status=0] */
...
func E_Generate_main_function_list (l_used EID) EID { 
    F_Generate_main_function_list(ToList(OBJ(l_used)) )
    return EVOID} 
