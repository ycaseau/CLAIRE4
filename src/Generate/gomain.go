...
func F_Generate_string2module_string (s *ClaireString ) EID { 
    ...
    /* Let:2 */{ 
      ...
      if (m.Isa.IsIn(C_module) == CTRUE) /* If:3 */{ 
        /* Let:4 */{ 
          var g0632 *ClaireModule   = ToModule(m)
          /* noccur = 1 */
          Result = EID{g0632.Id(),0}
          /* Let-4 */} 
        } else {
                  ...
                  /* Let:9 */{ 
                    ...
                    for (l.Length() != 0) /* while:10 */{ 
                      ...
                      if ((ToString(l.ValuesO()[1-1]).Value == MakeString("?").Value) || 
                          (ToString(l.ValuesO()[1-1]).Value == MakeString("-help").Value)) /* If:11 */{ 
                        ...
                        if (l.Length() >= 2) /* If:12 */{ 
                          ...
                          /* Let:13 */{ 
                            ...
                            /* noccur = 3 */
                            var m_try063614 EID 
                            m_try063614 = F_Generate_string2module_string(ToString(l.ValuesO()[2-1]))
                            /* ERROR PROTECTION INSERTED (m-void_try11) */
                            if ErrorIn(m_try063614) {void_try11 = m_try063614
                            } else {
                            m = ToModule(OBJ(m_try063614))
                        ...
                        if (l.Length() >= 2) /* If:12 */{ 
                          /* update:13 */{ 
                            ...
                            var va_arg1_try063714 EID 
                            va_arg1_try063714 = Core.F_new_class2(Core.C_global_variable,Core.F_symbol_I_string2(ToString(l.ValuesO()[2-1])))
                            /* ERROR PROTECTION INSERTED (va_arg1-void_try11) */
                            if ErrorIn(va_arg1_try063714) {void_try11 = va_arg1_try063714
                            } else {
                            va_arg1 = Core.ToGlobalVariable(OBJ(va_arg1_try063714))
                            va_arg2 = CTRUE.Id()
                    ...
                    if (_Zcx.Value != MakeString("").Value) /* If:10 */{ 
                      ...
                      if !ErrorIn(Result) {
                      /* Let:11 */{ 
                        var g0638UU *ClaireModule  
                        /* noccur = 1 */
                        var g0638UU_try063912 EID 
                        g0638UU_try063912 = F_Generate_string2module_string(_Zcx)
                        /* ERROR PROTECTION INSERTED (g0638UU-Result) */
                        if ErrorIn(g0638UU_try063912) {Result = g0638UU_try063912
                        } else {
                        g0638UU = ToModule(OBJ(g0638UU_try063912))
                        Result = F_system_file_module(g0638UU,_Zout)
                        }
                        /* Let-11 */} 
                      /* ERROR PROTECTION INSERTED (Result-Result) */
                      if !ErrorIn(Result) {
                      ...
                      /* Let:11 */{ 
                        ...
                        /* noccur = 10 */
                        var m_try064012 EID 
                        m_try064012 = F_Generate_string2module_string(_Zcm)
                        /* ERROR PROTECTION INSERTED (m-Result) */
                        if ErrorIn(m_try064012) {Result = m_try064012
                        } else {
                        m = ToModule(OBJ(m_try064012))
                        Optimize.C_compiler.Active_ask = CTRUE
...
func F_system_file_module (m *ClaireModule ,_Zout *ClaireString ) EID { 
    ...
    /* Let:2 */{ 
      ...
      /* noccur = 2 */
      var p_try06413 EID 
      p_try06413 = F_fopen_string(F_append_string(Reader.F__7_string(Optimize.C_compiler.Source,_Zout),ToGenerateCodeProducer(Optimize.C_PRODUCER.Value).Extension),MakeString("w"))
      /* ERROR PROTECTION INSERTED (p-Result) */
      if ErrorIn(p_try06413) {Result = p_try06413
      } else {
      p = ToPort(OBJ(p_try06413))
      /* Let:3 */{ 
    ...
    /* For:2 */{ 
      ...
      for i_it := 0; i_it < x_len; i_it++ { 
        ...
        if ((x != C_mClaire.Id()) && 
          ...
          PRINC(",")
          /* Let:5 */{ 
            var g0642UU *ClaireAny  
            /* noccur = 1 */
            var g0642UU_try06436 EID 
            g0642UU_try06436 = Core.F_CALL(Optimize.C_c_code,ARGS(EID{ToModule(x).Uses.Id(),0},EID{C_list.Id(),0}))
            /* ERROR PROTECTION INSERTED (g0642UU-void_try4) */
            if ErrorIn(g0642UU_try06436) {void_try4 = g0642UU_try06436
            } else {
            g0642UU = ANY(g0642UU_try06436)
            void_try4 = Core.F_CALL(C_Generate_g_expression,ARGS(g0642UU.ToEID(),EID{C_list.Id(),0}))
            }
            /* Let-5 */} 
          /* ERROR PROTECTION INSERTED (void_try4-void_try4) */
          if ErrorIn(void_try4) {Result = void_try4
          ...
          PRINC(",\n\t")
          /* Let:5 */{ 
            var g0644UU *ClaireAny  
            /* noccur = 1 */
            var g0644UU_try06456 EID 
            g0644UU_try06456 = Core.F_CALL(Optimize.C_c_code,ARGS(EID{ToModule(x).MadeOf.Id(),0},EID{C_list.Id(),0}))
            /* ERROR PROTECTION INSERTED (g0644UU-void_try4) */
            if ErrorIn(g0644UU_try06456) {void_try4 = g0644UU_try06456
            } else {
            g0644UU = ANY(g0644UU_try06456)
            void_try4 = Core.F_CALL(C_Generate_g_expression,ARGS(g0644UU.ToEID(),EID{C_list.Id(),0}))
            }
            /* Let-5 */} 
          /* ERROR PROTECTION INSERTED (void_try4-void_try4) */
          if ErrorIn(void_try4) {Result = void_try4
