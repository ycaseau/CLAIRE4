                ...
                if (r.Firstc() == 60) { 
                  { var y *ClaireAny  
                    ...
                    if ((C_class.Id() == x.Isa.Id()) && 
                      ...
                      r.Cnext()
                      var try_11 EID 
                      /*g_try(v2:"try_11",loop:tuple("Result", EID)) */
                      { var arg_12 *ClaireList  
                        _ = arg_12
                        { 
                          var v_bag_arg *ClaireAny  
                          arg_12= ToType(CEMPTY.Id()).EmptyList()
                          { var _CL_obj *Language.Call   = Language.To_Call(new(Language.Call).Is(Language.C_Call))
                            _CL_obj.Selector = ToProperty(C__equal.Id())
                            /*property->property*/_CL_obj.Args = MakeConstantList(C_of.Id(),y)
                            /*list->list*/v_bag_arg = _CL_obj.Id()
                            } 
                          arg_12.AddFast(v_bag_arg)} 
                        try_11 = Language.F_extract_class_call_class(ToClass(x),arg_12)
                        } 
                      ...
                      try_13 = r.Nexti(x)
                      /* ERROR PROTECTION INSERTED (x-loop_9) */
                      if ErrorIn(try_13) {loop_9 = try_13
                      Result = try_13
                      break
                      } else {
                      x = ANY(try_13)
                  ...
                  { var l *ClaireAny  
                    ...
                    try_14 = r.Cnext().Nextseq(93)
                    /* ERROR PROTECTION INSERTED (l-loop_9) */
                    ...
                    if ((C_class.Id() == x.Isa.Id()) && 
                      ...
                      try_15 = Language.F_extract_class_call_class(ToClass(x),ToList(l))
                      } else {
                      try_15 = F_Call_I_property(C_nth,F_cons_any(x,ToList(l)))
                      } 
                    /* ERROR PROTECTION INSERTED (x-loop_9) */
    ...
    if (r.Firstc() == 40) { 
      ...
      { var s *ClaireAny  
        ...
        if (s.Isa.IsIn(Language.C_Image) == CTRUE) { 
          { var g0011 *Language.Image   = Language.To_Image(s)
            g0011.Isa = Language.C_Collect
            /*class->class*/Result = EID{g0011.Id(),0}
            } 
          }  else if (s.Isa.IsIn(Language.C_Select) == CTRUE) { 
          { var g0012 *Language.Select   = Language.To_Select(s)
            g0012.Isa = Language.C_Lselect
            /*class->class*/Result = EID{g0012.Id(),0}
            } 
          } else {
      ...
      { 
        ...
        if (val.Isa.IsIn(Language.C_Call) == CTRUE) { 
          { var g0014 *Language.Call   = Language.To_Call(val)
            v_and3 = MakeBoolean((g0014.Selector.Id() == C_nth.Id()) && (g0014.Args.At(1-1) == C_list.Id()))
            } 
          } else {
          v_and3 = CFALSE
          } 
        if (v_and3 == CFALSE) {g0025I = CFALSE
        } else { 
      ...
      if (g0025I == CTRUE) { 
          ...
          { var x *ClaireType  
            ...
            if (s.Isa.IsIn(Language.C_Image) == CTRUE) { 
              { var g0016 *Language.Image   = Language.To_Image(s)
                ...
                /*class->class*/g0016.Of = x
                /*type->type*/Result = EID{g0016.Id(),0}
                } 
              }  else if (s.Isa.IsIn(Language.C_Select) == CTRUE) { 
              { var g0017 *Language.Select   = Language.To_Select(s)
                g0017.Isa = Language.C_Lselect
                /*class->class*/g0017.Of = x
                /*type->type*/Result = EID{g0017.Id(),0}
                } 
              } else {
              Result = F_Serror_string(MakeString("[157] ~S cannot follow list{"),MakeConstantList(s))
              } 
        ...
        var g0026I *ClaireBoolean  
        { 
          var v_and4 *ClaireBoolean  
          
          if (val.Isa.IsIn(Language.C_Call) == CTRUE) { 
            { var g0019 *Language.Call   = Language.To_Call(val)
              v_and4 = MakeBoolean((g0019.Selector.Id() == C_nth.Id()) && (g0019.Args.At(1-1) == C_set.Id()))
              } 
            } else {
            v_and4 = CFALSE
            } 
          if (v_and4 == CFALSE) {g0026I = CFALSE
          } else { 
...
func (r *MetaReader ) Nexte_I (e *ClaireKeyword ) EID { 
    ...
    { var x *ClaireAny  
      ...
      /* ERROR PROTECTION INSERTED (g0027I-Result) */
      if ErrorIn(try_2) {Result = try_2
      } else {
      g0027I = ToBoolean(OBJ(try_2))
      if (g0027I == CTRUE) { 
        Result = x.ToEID()
        } else {
        Result = F_Serror_string(MakeString("[161] Missing keyword ~S after ~S"),MakeConstantList(e.Id(),x))
