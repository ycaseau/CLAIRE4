/***** CLAIRE Compilation of file /Users/ycaseau/Dropbox/src/clairev4.10/src/compile/otool.cl 
...
func F__Z_any3 (x *ClaireAny,y *ClairePattern) EID { 
    ...
    if (x.Isa.IsIn(Language.C_Call) == CTRUE) { 
      { var g0099 *Language.Call = Language.To_Call(x)
        { 
          var v_and4 *ClaireBoolean
          
          v_and4 = Equal(g0099.Selector.Id(),y.Selector.Id())
...
func F_glb_Pattern (x *ClairePattern,y *ClaireTypeExpression) *ClaireTypeExpression { 
    ...
    if (y.Isa.IsIn(C_Optimize_Pattern) == CTRUE) { 
      { var g0101 *ClairePattern = To_ClairePattern(y.Id())
        if (x.Selector.Id() == g0101.Selector.Id()) { 
          { var _CL_obj *ClairePattern = To_ClairePattern(new(ClairePattern).Is(C_Optimize_Pattern))
            _CL_obj.Selector = x.Selector
            _CL_obj.Arg = ToList(OBJ(Core.F_CALL(ToProperty(Core.C_glb.Id()),ARGS(EID{x.Arg.Id(),0},EID{g0101.Arg.Id(),0}))))
...
func F_glb_type_expression (x *ClaireTypeExpression,y *ClairePattern) *ClaireTypeExpression { 
    ...
    if (x.Isa.IsIn(C_Optimize_Pattern) == CTRUE) { 
      { var g0103 *ClairePattern = To_ClairePattern(x.Id())
        if (g0103.Selector.Id() == y.Selector.Id()) { 
          { var _CL_obj *ClairePattern = To_ClairePattern(new(ClairePattern).Is(C_Optimize_Pattern))
            _CL_obj.Selector = g0103.Selector
            _CL_obj.Arg = ToList(OBJ(Core.F_CALL(ToProperty(Core.C_glb.Id()),ARGS(EID{g0103.Arg.Id(),0},EID{y.Arg.Id(),0}))))
            Result = ToTypeExpression(_CL_obj.Id())
            } 
          } else {
          Result = ToTypeExpression(CEMPTY.Id())
...
func F_less_ask_Pattern (x *ClairePattern,y *ClaireTypeExpression) *ClaireBoolean { 
    ...
    if (y.Isa.IsIn(C_Optimize_Pattern) == CTRUE) { 
      { var g0105 *ClairePattern = To_ClairePattern(y.Id())
        { 
          var v_and4 *ClaireBoolean
          
          v_and4 = Equal(x.Selector.Id(),g0105.Selector.Id())
          if (v_and4 == CFALSE) {Result = CFALSE
          } else { 
            v_and4 = Equal(MakeInteger(x.Arg.Length()).Id(),MakeInteger(g0105.Arg.Length()).Id())
            if (v_and4 == CFALSE) {Result = CFALSE
            } else { 
              { var arg_1 *ClaireAny
                { var i int = 1
                  { var g0106 int = x.Arg.Length()
                    arg_1= CFALSE.Id()
                    for (i <= g0106) { 
                      if (Core.F__equaltype_ask_any(ToType(x.Arg.At(i-1)),ToType(g0105.Arg.At(i-1))) != CTRUE) { 
                        arg_1 = CTRUE.Id()
                        break
                        } 
                      i = (i+1)
...
func F_less_ask_type_expression2 (x *ClaireTypeExpression,y *ClairePattern) EID { 
    ...
    if (C_set.Id() == x.Isa.Id()) { 
      { var g0108 *ClaireSet = ToSet(x.Id())
        { var arg_1 *ClaireAny
          var try_2 EID
          { 
            var z *ClaireAny
      ...
      { var g0109 *ClairePattern = To_ClairePattern(x.Id())
        { 
          var v_and4 *ClaireBoolean
          
          v_and4 = Equal(g0109.Selector.Id(),y.Selector.Id())
          if (v_and4 == CFALSE) {Result = EID{CFALSE.Id(),0}
          } else { 
            v_and4 = Equal(MakeInteger(g0109.Arg.Length()).Id(),MakeInteger(y.Arg.Length()).Id())
            if (v_and4 == CFALSE) {Result = EID{CFALSE.Id(),0}
            } else { 
              { var arg_7 *ClaireAny
                { var i int = 1
                  { var g0110 int = g0109.Arg.Length()
                    arg_7= CFALSE.Id()
                    for (i <= g0110) { 
                      if (Core.F__equaltype_ask_any(ToType(g0109.Arg.At(i-1)),ToType(y.Arg.At(i-1))) != CTRUE) { 
                        arg_7 = CTRUE.Id()
                        break
                        } 
                      i = (i+1)
      ...
      if (_Ztype == C_void.Id()) { 
        Result = Core.F_CALL(C_Compile_Cerror,ARGS(EID{MakeString("[205] message ~S sent to void object").Id(),0},EID{self.Id(),0}))
          ...
          ((C_OPT.Ignore.Contain_ask(s.Id()) != CTRUE) && 
            (s.Open < ClEnv.Open))) { 
        Result = Core.F_CALL(C_Compile_Cerror,ARGS(EID{MakeString("[255] property ~S has no definition and is not defined as open").Id(),0},EID{s.Id(),0}))
        } else {
        ...
        { 
          ...
          if (v_and4 == CFALSE) {g0116I = CFALSE
          } else { 
            v_and4 = MakeBoolean((s.Open <= 1) || (s.Open == 4))
            if (v_and4 == CFALSE) {g0116I = CFALSE
            } else { 
              if (_Ztype.Isa.IsIn(C_list) == CTRUE) { 
                { var g0115 *ClaireList = ToList(_Ztype)
                  v_and4 = Core.F__I_equal_any(MakeInteger(ToTypeExpression(g0115.At(0)).Class_I().Open).Id(),MakeInteger(3).Id())
                  } 
                } else {
                v_and4 = CFALSE
                } 
...
func F_sort_Variable (x *ClaireVariable) *ClaireClass { 
    ...
    { var r *ClaireType = x.Range
      ...
      if (r.Isa.IsIn(C_Union) == CTRUE) { 
        { var g0118 *ClaireUnion = To_Union(r.Id())
          g0119I = Equal(g0118.T1.Id(),CEMPTY.Id())
          } 
        } else {
        g0119I = CFALSE
        } 
...
func F_Optimize_extended_ask_type (self *ClaireType) EID { 
    ...
    if (self.Isa.IsIn(C_Union) == CTRUE) { 
      { var g0120 *ClaireUnion = To_Union(self.Id())
        { 
          var v_and4 *ClaireBoolean
          
          v_and4 = Equal(C_set.Id(),g0120.T2.Isa.Id())
          if (v_and4 == CFALSE) {Result = EID{CFALSE.Id(),0}
          } else { 
            var try_1 EID
            { var arg_2 *ClaireAny
              var try_3 EID
              try_3 = Core.F_CALL(C_size,ARGS(EID{g0120.T2.Id(),0}))
              if ErrorIn(try_3) {try_1 = try_3
              } else {
              arg_2 = ANY(try_3)
            ...
            if (v_and4 == CFALSE) {Result = EID{CFALSE.Id(),0}
              ...
              { var arg_5 *ClaireAny
                var try_6 EID
                try_6 = Core.F_the_type(g0120.T2)
                if ErrorIn(try_6) {try_4 = try_6
                } else {
                arg_5 = ANY(try_6)
...
func F_Optimize_sort_abstract_ask_type (x *ClaireType) *ClaireBoolean { 
    ...
    if (x.Isa.IsIn(C_Union) == CTRUE) { 
      { var g0122 *ClaireUnion = To_Union(x.Id())
        Result = Equal(g0122.T1.Id(),C_any.Id())
        } 
      } else {
      Result = CFALSE
      } 
...
func F_Optimize_ptype_type (x *ClaireType) *ClaireType { 
    var Result *ClaireType
    if (x.Isa.IsIn(C_Union) == CTRUE) { 
      { var g0124 *ClaireUnion = To_Union(x.Id())
        if (g0124.T1.Id() == C_any.Id()) { 
          Result = g0124.T2
          } else {
          Result = ToType(g0124.Id())
          } 
        } 
      } else {
      Result = x
...
func F_Optimize_range_sets_any (self *ClaireAny,y *ClaireType)  { 
    if (self.Isa.IsIn(C_Variable) == CTRUE) { 
      { var g0127 *ClaireVariable = To_Variable(self)
        g0127.Range = y
        } 
      } 
    } 
...
func F_Compile_return_type_any (self *ClaireAny) EID { 
    ...
    if (self.Isa.IsIn(Language.C_Let) == CTRUE) { 
      { var g0128 *Language.Let = Language.To_Let(self)
        Result = F_Compile_return_type_any(g0128.Arg)
        } 
      }  else if (self.Isa.IsIn(Language.C_Do) == CTRUE) { 
      { var g0129 *Language.Do = Language.To_Do(self)
      ...
      { var g0130 *Language.If = Language.To_If(self)
        { var arg_5 *ClaireType
          var try_7 EID
          try_7 = F_Compile_return_type_any(g0130.Arg)
          if ErrorIn(try_7) {Result = try_7
          } else {
          arg_5 = ToType(OBJ(try_7))
          { var arg_6 *ClaireType
            var try_8 EID
            try_8 = F_Compile_return_type_any(g0130.Other)
      ...
      { var g0131 *Language.Return = Language.To_Return(self)
        Result = Core.F_CALL(C_c_type,ARGS(g0131.Arg.ToEID()))
        } 
      }  else if (self.Isa.IsIn(Language.C_Case) == CTRUE) { 
      { var g0132 *Language.Case = Language.To_Case(self)
        { var x *ClaireSet = CEMPTY
          { 
            var y *ClaireAny
            _ = y
      ...
      { var g0133 *Language.ClaireHandle = Language.To_ClaireHandle(self)
        Result = F_Compile_return_type_any(g0133.Arg)
        } 
      } else {
          ...
          { var arg_2 *ClaireList
            ...
            { var arg_3 *ClaireList
              { var i_bag *ClaireList = ToType(CEMPTY.Id()).EmptyList()
                { var i int = 1
                  { var g0136 int = self.Params.Length()
                    for (i <= g0136) { 
            ...
            if (v_or4 == CTRUE) {g0139I = CTRUE
              ...
              { var arg_5 *ClaireObject
                if (self.Isa.IsIn(Core.C_global_variable) == CTRUE) { 
                  { var g0138 *Core.GlobalVariable = Core.ToGlobalVariable(self)
                    arg_5 = ToObject(g0138.Range.Id())
                    } 
                  } else {
                  ...
                  if (v_or2 == CTRUE) {Result = EID{CTRUE.Id(),0}
                    ...
                    if (v_or2 == CTRUE) {Result = EID{CTRUE.Id(),0}
                      ...
                      if (self.Isa.IsIn(Language.C_Call) == CTRUE) { 
                        { var g0142 *Language.Call = Language.To_Call(self)
                          { var x *ClaireAny
                            var try_2 EID
                            try_2 = Core.F_CALL(C_c_code,ARGS(EID{g0142.Id(),0}))
                            if ErrorIn(try_2) {try_1 = try_2
                            } else {
                            x = ANY(try_2)
                            { 
                              ...
                              if (v_or14 == CTRUE) {try_1 = EID{CTRUE.Id(),0}
                                ...
                                v_or14 = Equal(g0142.Selector.Id(),Core.C_mClaire_get_stack.Id())
                                if (v_or14 == CTRUE) {try_1 = EID{CTRUE.Id(),0}
                                } else { 
                                  try_1 = EID{CFALSE.Id(),0}} 
                                } 
                        ...
                        { var g0143 *Language.CallSlot = Language.To_CallSlot(self)
                          try_1 = F_Compile_designated_ask_any(g0143.Arg)
                          } 
                        }  else if (self.Isa.IsIn(Language.C_Call_table) == CTRUE) { 
                        { var g0144 *Language.CallTable = Language.To_CallTable(self)
                          try_1 = F_Compile_designated_ask_any(g0144.Arg)
                          } 
                        }  else if (self.Isa.IsIn(Language.C_Call_array) == CTRUE) { 
                        { var g0145 *Language.CallArray = Language.To_CallArray(self)
                          try_1 = F_Compile_designated_ask_any(g0145.Arg)
                          } 
                        }  else if (self.Isa.IsIn(Language.C_Call_method) == CTRUE) { 
                        { var g0146 *Language.CallMethod = Language.To_CallMethod(self)
                          { 
                            var v_and13 *ClaireBoolean
                            
                            v_and13 = MakeBoolean((C_OPT.SimpleOperations.Contain_ask(g0146.Arg.Selector.Id()) == CTRUE) || (g0146.Arg.Id() == Core.F__at_property1(Core.C_unsafe,C_any).Id()) || (g0146.Arg.Id() == Core.F__at_property1(C_nth,C_list).Id()))
...
func F_Optimize_c_inline_arg_ask_any (self *ClaireAny) EID { 
    var Result EID
    if (self.Isa.IsIn(Language.C_Call) == CTRUE) { 
      { var g0150 *Language.Call = Language.To_Call(self)
        { var l *ClaireList = g0150.Args
          { var m *ClaireAny
            var try_1 EID
            { var arg_2 *ClaireList
              var try_3 EID
              ...
              arg_2 = ToList(OBJ(try_3))
              try_1 = F_Optimize_restriction_I_property(g0150.Selector,arg_2,CTRUE).ToEID()
              }
              } 
            if ErrorIn(try_1) {Result = try_1
            } else {
            ...
            if (C_method.Id() == m.Isa.Id()) { 
              { var g0151 *ClaireMethod = ToMethod(m)
                var g0154I *ClaireBoolean
                var try_5 EID
                { 
                  var v_and8 *ClaireBoolean
                  
                  v_and8 = g0151.Inline_ask
                  if (v_and8 == CFALSE) {try_5 = EID{CFALSE.Id(),0}
                  } else { 
                    var try_6 EID
                    try_6 = F_Optimize_c_inline_ask_method(g0151,l)
                    if ErrorIn(try_6) {try_5 = try_6
                    } else {
                ...
                g0154I = ToBoolean(OBJ(try_5))
                if (g0154I == CTRUE) { 
                  Result = F_Optimize_c_inline_method2(g0151,l)
                  } else {
                  Result = EID{CFALSE.Id(),0}
                  } 
                }
...
func F_Optimize_c_substitution_any (self *ClaireAny,lx *ClaireList,val *ClaireList,eval_ask *ClaireBoolean) EID { 
    var Result EID
    if (self.Isa.IsIn(C_Variable) == CTRUE) { 
      { var g0155 *ClaireVariable = To_Variable(self)
        { var i *ClaireAny
          { var j_some *ClaireAny = CNULL
            { var j int = 1
              { var g0156 int = lx.Length()
                for (j <= g0156) { 
                  if (g0155.Pname.Id() == To_Variable(lx.At(j-1)).Pname.Id()) { 
        ...
        { var i int = 1
          { var g0158 int = g0157.Length()
            ...
            for (i <= g0158) { 
              ...
              { var arg_2 *ClaireAny
                var try_3 EID
                try_3 = F_Optimize_c_substitution_any(g0157.At(i-1),lx,val,eval_ask)
                if ErrorIn(try_3) {loop_1 = try_3
                } else {
                arg_2 = ANY(try_3)
                loop_1 = ToArray(g0157.Id()).NthPut(i,arg_2).ToEID()
                }
                } 
      ...
      { var g0159 *Language.Call = Language.To_Call(self)
        if (g0159.Selector.Id() == Core.C_eval.Id()) { 
          { var arg_4 *ClaireBoolean
            var try_5 EID
            { 
              var v_or6 *ClaireBoolean
              
              v_or6 = Equal(MakeInteger(g0159.Args.Length()).Id(),MakeInteger(1).Id())
              if (v_or6 == CTRUE) {try_5 = EID{CTRUE.Id(),0}
                ...
                { 
                  ...
                  v_and8 = Equal(MakeInteger(g0159.Args.Length()).Id(),MakeInteger(2).Id())
                  if (v_and8 == CFALSE) {try_6 = EID{CFALSE.Id(),0}
                  } else { 
                    var try_7 EID
                    try_7 = Core.F_BELONG(val.At(0),g0159.Args.At(1))
                    if ErrorIn(try_7) {try_6 = try_7
                    } else {
            ...
            Result = F_Optimize_c_substitution_any(g0159.Args.At(0),lx,val,arg_4)
            }
            } 
          ...
          { 
            ...
            { var arg_8 *ClaireList
              ...
              Result = Core.F_apply_property(g0159.Selector,arg_8)
              }
              } 
            if ErrorIn(Result){ 
              ClEnv.Index = h_index
              ...
              Result = F_Optimize_c_substitution_any(g0159.Args.Id(),lx,val,CFALSE)
              if !ErrorIn(Result) {
      ...
      { var g0160 *ClaireInstruction = To_Instruction(self)
        { 
          ...
          s_support = g0160.Id().Isa.Slots
          for _,s_iter = range(s_support.ValuesO()){ 
            s = ToSlot(s_iter)
            var loop_11 EID
            _ = loop_11
            { var y *ClaireAny = Core.F_get_slot(s,ToObject(g0160.Id()))
              { var arg_12 *ClaireAny
                ...
                arg_12 = ANY(try_13)
                loop_11 = Core.F_put_slot(s,ToObject(g0160.Id()),arg_12).ToEID()
                }
                } 
              } 
...
func F_Optimize_bound_variables_any (self *ClaireAny) EID { 
    ...
    { var l *ClaireList = ToType(C_any.Id()).EmptyList()
      if (self.Isa.IsIn(Language.C_Instruction_with_var) == CTRUE) { 
        { var g0162 *Language.InstructionWithVar = Language.To_InstructionWithVar(self)
          l = MakeList(ToType(C_any.Id()),g0162.ClaireVar.Id())
          } 
        } 
      if (self.Isa.IsIn(C_Variable) == CTRUE) { 
        Result = EID{CNIL.Id(),0}
        }  else if (self.Isa.IsIn(C_Instruction) == CTRUE) { 
        { var g0164 *ClaireInstruction = To_Instruction(self)
          { 
            var s *ClaireSlot
            _ = s
            var s_iter *ClaireAny
            Result= EID{CFALSE.Id(),0}
            for _,s_iter = range(g0164.Isa.Slots.ValuesO()){ 
              s = ToSlot(s_iter)
              var loop_1 EID
              _ = loop_1
              var try_2 EID
              { var arg_3 *ClaireList
                var try_4 EID
                try_4 = F_Optimize_bound_variables_any(Core.F_get_slot(s,ToObject(g0164.Id())))
                if ErrorIn(try_4) {try_2 = try_4
                } else {
        ...
        { var g0165 *ClaireList = ToList(self)
          { 
            var x *ClaireAny
            _ = x
            Result= EID{CFALSE.Id(),0}
    ...
    { var tx *ClaireType
      ...
      { var ptx *ClaireType = F_Optimize_ptype_type(tx)
        if (ptx.Included(ToType(C_boolean.Id())) == CTRUE) { 
          if (x.Isa.IsIn(Language.C_Call) == CTRUE) { 
            { var g0166 *Language.Call = Language.To_Call(x)
              var g0167I *ClaireBoolean
              var try_2 EID
              { 
                var v_and7 *ClaireBoolean
                
                v_and7 = Equal(g0166.Selector.Id(),Core.C_not.Id())
                if (v_and7 == CFALSE) {try_2 = EID{CFALSE.Id(),0}
                  ...
                  { var arg_4 *ClaireType
                    ...
                    { var arg_6 *ClaireType
                      var try_7 EID
                      try_7 = Core.F_CALL(C_c_type,ARGS(g0166.Args.At(0).ToEID()))
                      if ErrorIn(try_7) {try_5 = try_7
                      } else {
                  ...
                  { 
                    ...
                    { 
                      ...
                      { var _CL_obj *Language.Call = Language.To_Call(new(Language.Call).Is(Language.C_Call))
                        ...
                        _CL_obj.Args = MakeConstantList(g0166.Args.At(0))
                        v_bag_arg = _CL_obj.Id()
                        } 
                      va_arg2.AddFast(v_bag_arg)
