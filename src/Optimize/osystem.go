...
func F_c_type_any (self *ClaireAny) EID { 
  ...
  if (self.Isa.IsIn(C_Variable) == CTRUE) { 
    ...
    { var g0004 *ClaireUnboundSymbol = ToUnboundSymbol(self)
      Result = F_Compile_Cerror_string(MakeString("[215] the symbol ~A is unbound_symbol"),ToList(MakeConstantList(g0004.Name.Id()).Id()))
      } 
...
func F_c_code_any1 (x *ClaireAny,s *ClaireClass) EID { 
  ...
  { var y *ClaireAny
    ...
    if (x.Isa.IsIn(Language.C_Call) == CTRUE) { 
      { var g0013 *Language.Call = Language.To_Call(x)
        try_1 = F_Optimize_c_code_call_Call(g0013,s)
        } 
      } else {
    ...
    { var z *ClaireClass
      ...
      if ((s.Id() == C_void.Id()) || 
          (z.Id() == s.Id())) { 
        var g0016I *ClaireBoolean
        { 
          var v_and4 *ClaireBoolean
          
          v_and4 = Equal(s.Id(),C_void.Id())
          if (v_and4 == CFALSE) {g0016I = CFALSE
          } else { 
            if (x.Isa.IsIn(Language.C_Call) == CTRUE) { 
              { var g0015 *Language.Call = Language.To_Call(x)
                v_and4 = Equal(g0015.Selector.Id(),C__equal.Id())
                } 
              } else {
              v_and4 = CFALSE
              } 
...
func F_c_code_any2 (self *ClaireAny) EID { 
  ...
  if (self.Isa.IsIn(C_unbound_symbol) == CTRUE) { 
    { var g0017 *ClaireUnboundSymbol = ToUnboundSymbol(self)
      Result = F_Compile_Cerror_string(MakeString("[215] the symbol ~A is unbound_symbol"),ToList(MakeConstantList(g0017.Name.Id()).Id()))
      } 
    }  else if (self.Isa.IsIn(C_Variable) == CTRUE) { 
    { var g0018 *ClaireVariable = To_Variable(self)
      Result = EID{g0018.Id(),0}
      } 
    }  else if (self.Isa.IsIn(Core.C_global_variable) == CTRUE) { 
    { var g0019 *Core.GlobalVariable = Core.ToGlobalVariable(self)
      F_Optimize_c_register_object(ToObject(g0019.Id()))
      Result = EID{g0019.Id(),0}
      } 
    }  else if (self.Isa.IsIn(Language.C_Optimized_instruction) == CTRUE) { 
    { var g0020 *Language.OptimizedInstruction = Language.To_OptimizedInstruction(self)
      Result = EID{g0020.Id(),0}
      } 
    }  else if (self.Isa.IsIn(C_Instruction) == CTRUE) { 
    { var g0021 *ClaireInstruction = To_Instruction(self)
      Result = F_Compile_Cerror_string(MakeString("[internal] c_code(~S) should have a parameter"),ToList(MakeConstantList(g0021.Id()).Id()))
      } 
    }  else if (C_set.Id() == self.Isa.Id()) { 
    { var g0022 *ClaireSet = ToSet(self)
      if (F_boolean_I_any(g0022.Id()) == CTRUE) { 
        { var x *Language.Set
          { var _CL_obj *Language.Set = Language.To_Set(new(Language.Set).Is(Language.C_Set))
            _CL_obj.Args = g0022.List_I()
            x = _CL_obj
            } 
          if (ToList(g0022.Id()).Of().Id() != C_void.Id()) { 
            x.Of = ToList(g0022.Id()).Of()
            } 
    ...
    { var g0023 *ClaireList = ToList(self)
      if (g0023.Length() != 0) { 
        { var x *Language.List
    ...
    { var g0024 *ClaireTuple = ToTuple(self)
...
func F_Compile_c_sort_any (self *ClaireAny) EID { 
  ...
  if (self.Isa.IsIn(Core.C_global_variable) == CTRUE) { 
    { var g0026 *Core.GlobalVariable = Core.ToGlobalVariable(self)
      if (F_Compile_nativeVar_ask_global_variable(g0026) == CTRUE) { 
        if (Equal(g0026.Range.Id(),CEMPTY.Id()) == CTRUE) { 
          Result = EID{F_Compile_osort_any(g0026.Value.Isa.Id()).Id(),0}
          } else {
          Result = EID{F_Compile_osort_any(g0026.Range.Id()).Id(),0}
          } 
        } else {
        Result = EID{C_any.Id(),0}
        } 
    ...
    { var g0027 *ClaireInstruction = To_Instruction(self)
      if (g0027.Isa.IsIn(C_Variable) == CTRUE) { 
        { var g0028 *ClaireVariable = To_Variable(g0027.Id())
          Result = EID{F_sort_Variable(g0028).Id(),0}
          } 
        }  else if (g0027.Isa.IsIn(Language.C_Assign) == CTRUE) { 
        { var g0029 *Language.Assign = Language.To_Assign(g0027.Id())
          Result = EID{F_sort_Variable(To_Variable(g0029.ClaireVar)).Id(),0}
          } 
        }  else if (g0027.Isa.IsIn(Language.C_Call) == CTRUE) { 
        { var g0030 *Language.Call = Language.To_Call(g0027.Id())
          Result = EID{F_Compile_osort_any(F_Optimize_selector_psort_Call(g0030).Id()).Id(),0}
          } 
        }  else if (g0027.Isa.IsIn(Language.C_Call_method) == CTRUE) { 
        { var g0031 *Language.CallMethod = Language.To_CallMethod(g0027.Id())
          if ((g0031.Arg.Selector.Id() == Core.C_externC.Id()) && 
              (g0031.Args.Length() == 2)) { 
            Result = EID{F_Compile_psort_any(g0031.Args.At(1)).Id(),0}
            } else {
            Result = F_Optimize_c_srange_method(g0031.Arg)
            } 
          } 
        }  else if (g0027.Isa.IsIn(Language.C_Call_slot) == CTRUE) { 
        { var g0032 *Language.CallSlot = Language.To_CallSlot(g0027.Id())
          Result = EID{g0032.Selector.Srange.Id(),0}
          } 
        }  else if (g0027.Isa.IsIn(Language.C_Call_table) == CTRUE) { 
        Result = EID{C_any.Id(),0}
        }  else if (g0027.Isa.IsIn(Language.C_Call_array) == CTRUE) { 
        { var g0034 *Language.CallArray = Language.To_CallArray(g0027.Id())
          if (g0034.Test == C_float.Id()) { 
            Result = EID{C_float.Id(),0}
            } else {
            Result = EID{C_any.Id(),0}
            } 
          } 
        }  else if (g0027.Isa.IsIn(Language.C_Definition) == CTRUE) { 
        Result = EID{C_object.Id(),0}
        }  else if (g0027.Isa.IsIn(C_Compile_C_cast) == CTRUE) { 
        { var g0036 *Compile_CCast = To_Compile_CCast(g0027.Id())
          Result = EID{g0036.SetArg.Id(),0}
          } 
        }  else if (g0027.Isa.IsIn(Language.C_Update) == CTRUE) { 
        { var g0037 *Language.Update = Language.To_Update(g0027.Id())
          Result = Core.F_CALL(C_Compile_c_sort,ARGS(g0037.Value.ToEID()))
          } 
        }  else if (g0027.Isa.IsIn(Language.C_If) == CTRUE) { 
        { var g0038 *Language.If = Language.To_If(g0027.Id())
          { var arg_1 *ClaireClass
            var try_2 EID
            { var arg_3 *ClaireAny
              var try_5 EID
              try_5 = Core.F_CALL(C_Compile_c_sort,ARGS(g0038.Arg.ToEID()))
              if ErrorIn(try_5) {try_2 = try_5
              } else {
              arg_3 = ANY(try_5)
              { var arg_4 *ClaireAny
                var try_6 EID
                try_6 = Core.F_CALL(C_Compile_c_sort,ARGS(g0038.Other.ToEID()))
        ...
        }  else if (g0027.Isa.IsIn(Language.C_Handle) == CTRUE) { 
        { var g0039 *Language.ClaireHandle = Language.To_ClaireHandle(g0027.Id())
          { var arg_7 *ClaireClass
            var try_8 EID
            { var arg_9 *ClaireAny
              var try_11 EID
              try_11 = Core.F_CALL(C_Compile_c_sort,ARGS(g0039.Arg.ToEID()))
              if ErrorIn(try_11) {try_8 = try_11
              } else {
              arg_9 = ANY(try_11)
              { var arg_10 *ClaireAny
                var try_12 EID
                try_12 = Core.F_CALL(C_Compile_c_sort,ARGS(g0039.Other.ToEID()))
        ...
        }  else if (g0027.Isa.IsIn(Language.C_Let) == CTRUE) { 
        { var g0040 *Language.Let = Language.To_Let(g0027.Id())
          Result = Core.F_CALL(C_Compile_c_sort,ARGS(g0040.Arg.ToEID()))
          } 
        }  else if (g0027.Isa.IsIn(Language.C_Return) == CTRUE) { 
        Result = EID{C_any.Id(),0}
        }  else if (g0027.Isa.IsIn(Language.C_List) == CTRUE) { 
        Result = EID{C_object.Id(),0}
        }  else if (g0027.Isa.IsIn(Language.C_Set) == CTRUE) { 
        Result = EID{C_object.Id(),0}
        }  else if (g0027.Isa.IsIn(Language.C_Tuple) == CTRUE) { 
        Result = EID{C_object.Id(),0}
        }  else if (g0027.Isa.IsIn(Language.C_Construct) == CTRUE) { 
        Result = EID{C_any.Id(),0}
        }  else if (g0027.Isa.IsIn(Language.C_Gassign) == CTRUE) { 
        Result = EID{C_any.Id(),0}
        }  else if (g0027.Isa.IsIn(Language.C_Super) == CTRUE) { 
        Result = EID{C_any.Id(),0}
        }  else if (g0027.Isa.IsIn(Language.C_For) == CTRUE) { 
        Result = EID{C_any.Id(),0}
        }  else if (g0027.Isa.IsIn(Language.C_Exists) == CTRUE) { 
        { var g0049 *Language.Exists = Language.To_Exists(g0027.Id())
          if (g0049.Other == CNULL) { 
            Result = EID{C_any.Id(),0}
            } else {
            Result = EID{C_object.Id(),0}
            } 
          } 
        }  else if (g0027.Isa.IsIn(Language.C_Iteration) == CTRUE) { 
        Result = EID{C_object.Id(),0}
        }  else if (g0027.Isa.IsIn(Language.C_And) == CTRUE) { 
        Result = EID{C_boolean.Id(),0}
        }  else if (g0027.Isa.IsIn(Language.C_Or) == CTRUE) { 
        Result = EID{C_boolean.Id(),0}
        }  else if (g0027.Isa.IsIn(Language.C_While) == CTRUE) { 
        Result = EID{C_any.Id(),0}
        }  else if (g0027.Isa.IsIn(Language.C_Do) == CTRUE) { 
        { var g0054 *Language.Do = Language.To_Do(g0027.Id())
          { var arg_13 *ClaireAny
            var try_14 EID
            try_14 = Core.F_last_list(g0054.Args)
        ...
        Result = ToException(Core.C_general_error.Make(MakeString("[internal] c_sort is not implemented for ~S").Id(),MakeConstantList(g0027.Id().Isa.Id()).Id())).Close()
        } 
      } 
...
func F_Optimize_selector_psort_Call (self *Language.Call) *ClaireClass { 
  ...
  { var p *ClaireProperty = self.Selector
    if ((p.Id() == Core.C_mClaire_base_I.Id()) || 
      ...
      Result = ToClass(self.Args.At(0))
      }  else if (p.Id() == C_Compile_object_I.Id()) { 
      Result = ToClass(self.Args.At(1))
      } else {
      Result = C_any
      } 
    } 
...
func F_Optimize_g_throw1_any (self *ClaireAny) EID { 
  ...
  if (self.Isa.IsIn(C_bag) == CTRUE) { 
    { var g0058 *ClaireBag = ToBag(self)
      { var arg_1 *ClaireAny
        var try_2 EID
        { 
          var x *ClaireAny
          ...
          var try_3 EID
          try_3 = Core.F_enumerate_any(g0058.Id())
    ...
    { var g0059 *Language.Construct = Language.To_Construct(self)
      { var arg_6 *ClaireAny
        var try_7 EID
        { 
          var x *ClaireAny
    ...
    { var g0060 *Language.Assign = Language.To_Assign(self)
      Result = F_Compile_g_throw_any(g0060.Arg)
      } 
    }  else if (self.Isa.IsIn(Language.C_Gassign) == CTRUE) { 
    { var g0061 *Language.Gassign = Language.To_Gassign(self)
      Result = F_Compile_g_throw_any(g0061.Arg)
      } 
    }  else if (self.Isa.IsIn(Language.C_And) == CTRUE) { 
    { var g0062 *Language.And = Language.To_And(self)
      Result = F_Compile_g_throw_any(g0062.Args.Id())
      } 
    }  else if (self.Isa.IsIn(Language.C_Or) == CTRUE) { 
    { var g0063 *Language.Or = Language.To_Or(self)
      Result = F_Compile_g_throw_any(g0063.Args.Id())
      } 
    }  else if (self.Isa.IsIn(Language.C_Call) == CTRUE) { 
    { var g0064 *Language.Call = Language.To_Call(self)
      { 
        var v_and3 *ClaireBoolean
        
        v_and3 = Core.F__I_equal_any(g0064.Selector.Id(),Core.C_unsafe.Id())
        if (v_and3 == CFALSE) {Result = EID{CFALSE.Id(),0}
          ...
          { 
            ...
            var try_11 EID
            try_11 = F_Compile_g_throw_any(g0064.Args.Id())
            if ErrorIn(try_11) {try_10 = try_11
            } else {
            v_or5 = ToBoolean(OBJ(try_11))
            if (v_or5 == CTRUE) {try_10 = EID{CTRUE.Id(),0}
            } else { 
              var try_12 EID
              try_12 = F_Compile_can_throw_ask_property(g0064.Selector)
              if ErrorIn(try_12) {try_10 = try_12
              } else {
              v_or5 = ToBoolean(OBJ(try_12))
              if (v_or5 == CTRUE) {try_10 = EID{CTRUE.Id(),0}
              } else { 
    ...
    { var g0065 *Language.CallMethod = Language.To_CallMethod(self)
      var g0084I *ClaireBoolean
      var try_13 EID
      { 
        var v_or3 *ClaireBoolean
        
        v_or3 = Equal(g0065.Arg.Id(),C_Compile_m_unsafe.Value)
        if (v_or3 == CTRUE) {try_13 = EID{CTRUE.Id(),0}
        } else { 
          var try_14 EID
          { var arg_15 *ClaireBoolean
            var try_16 EID
            try_16 = F_Optimize_notOpt_Call_method(g0065)
            if ErrorIn(try_16) {try_14 = try_16
            } else {
            arg_15 = ToBoolean(OBJ(try_16))
            try_14 = EID{Equal(arg_15.Id(),CFALSE.Id()).Id(),0}
            }
            } 
          if ErrorIn(try_14) {try_13 = try_14
          } else {
          v_or3 = ToBoolean(OBJ(try_14))
          if (v_or3 == CTRUE) {try_13 = EID{CTRUE.Id(),0}
          } else { 
            v_or3 = Equal(g0065.Arg.Selector.Id(),Core.C_externC.Id())
            if (v_or3 == CTRUE) {try_13 = EID{CTRUE.Id(),0}
            } else { 
      ...
      g0084I = ToBoolean(OBJ(try_13))
      if (g0084I == CTRUE) { 
        Result = EID{CFALSE.Id(),0}
        } else {
        var g0085I *ClaireBoolean
        var try_17 EID
        try_17 = F_Compile_g_throw_any(g0065.Args.Id())
        ...
        if (g0085I == CTRUE) { 
          ...
          Result = EID{MakeBoolean((C_compiler.Safety < 2) && ((C_integer.Id() != g0065.Args.At(1).Isa.Id()) || 
              g0065.Args.At(1).IsInt(0))).Id(),0}
          }  else if (g0065.Arg.Id() == C_Optimize__starmod_integer_star.Value) { 
          Result = EID{MakeBoolean((C_compiler.Safety < 2) && ((C_integer.Id() != g0065.Args.At(1).Isa.Id()) || 
              g0065.Args.At(1).IsInt(0))).Id(),0}
          }  else if (g0065.Arg.Id() == C_Optimize__stardiv_float_star.Value) { 
          Result = EID{MakeBoolean((C_compiler.Safety < 2) && ((C_float.Id() != g0065.Args.At(1).Isa.Id()) || 
              (Equal(g0065.Args.At(1),MakeFloat(0).Id()) == CTRUE))).Id(),0}
          } else {
          Result = F_Compile_can_throw_ask_method(g0065.Arg)
          } 
        }
        } 
      }
    ...
    { var g0066 *Language.CallSlot = Language.To_CallSlot(self)
      { 
        var v_or3 *ClaireBoolean
        
        var try_18 EID
        try_18 = F_Compile_g_throw_any(g0066.Arg)
        if ErrorIn(try_18) {Result = try_18
        } else {
        v_or3 = ToBoolean(OBJ(try_18))
        if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
        } else { 
          v_or3 = MakeBoolean((g0066.Test.Id() != CNULL) && (g0066.Test == CTRUE))
          if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
          } else { 
            Result = EID{CFALSE.Id(),0}} 
          } 
    ...
    { var g0067 *Language.CallTable = Language.To_CallTable(self)
      { 
        var v_or3 *ClaireBoolean
        
        var try_19 EID
        try_19 = F_Compile_g_throw_any(g0067.Arg)
        if ErrorIn(try_19) {Result = try_19
        } else {
        v_or3 = ToBoolean(OBJ(try_19))
        if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
        } else { 
          v_or3 = MakeBoolean((g0067.Test.Id() != CNULL) && (g0067.Test == CTRUE))
          if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
          } else { 
            Result = EID{CFALSE.Id(),0}} 
          } 
    ...
    { var g0068 *Language.CallArray = Language.To_CallArray(self)
      { 
        var v_or3 *ClaireBoolean
        
        var try_20 EID
        try_20 = F_Compile_g_throw_any(g0068.Selector)
        if ErrorIn(try_20) {Result = try_20
        } else {
        v_or3 = ToBoolean(OBJ(try_20))
        if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
        } else { 
          var try_21 EID
          try_21 = F_Compile_g_throw_any(g0068.Arg)
          if ErrorIn(try_21) {Result = try_21
          } else {
          v_or3 = ToBoolean(OBJ(try_21))
          if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
          } else { 
            Result = EID{CFALSE.Id(),0}} 
          } 
    ...
    { var g0069 *Language.Super = Language.To_Super(self)
      { 
        var v_or3 *ClaireBoolean
        
        var try_22 EID
        try_22 = F_Compile_g_throw_any(g0069.Args.Id())
        if ErrorIn(try_22) {Result = try_22
        } else {
        v_or3 = ToBoolean(OBJ(try_22))
        if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
        } else { 
          var try_23 EID
          try_23 = F_Compile_can_throw_ask_property(g0069.Selector)
          if ErrorIn(try_23) {Result = try_23
          } else {
          v_or3 = ToBoolean(OBJ(try_23))
          if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
          } else { 
            Result = EID{CFALSE.Id(),0}} 
          } 
    ...
    { var g0070 *Language.Update = Language.To_Update(self)
      { 
        var v_or3 *ClaireBoolean
        
        var try_24 EID
        try_24 = F_Compile_g_throw_any(g0070.Value)
        if ErrorIn(try_24) {Result = try_24
        } else {
        v_or3 = ToBoolean(OBJ(try_24))
        if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
        } else { 
          var try_25 EID
          try_25 = F_Compile_g_throw_any(g0070.ClaireVar)
          if ErrorIn(try_25) {Result = try_25
          } else {
          v_or3 = ToBoolean(OBJ(try_25))
          if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
          } else { 
            v_or3 = F_Compile_update_write_ask_Update(g0070)
            if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
            } else { 
              Result = EID{CFALSE.Id(),0}} 
            } 
    ...
    { var g0071 *Language.Cast = Language.To_Cast(self)
      Result = F_Compile_g_throw_any(g0071.Arg)
      } 
    }  else if (self.Isa.IsIn(C_Compile_C_cast) == CTRUE) { 
    { var g0072 *Compile_CCast = To_Compile_CCast(self)
      Result = F_Compile_g_throw_any(g0072.Arg)
      } 
    }  else if (self.Isa.IsIn(Language.C_Let) == CTRUE) { 
    { var g0073 *Language.Let = Language.To_Let(self)
      { 
        var v_or3 *ClaireBoolean
        
        var try_26 EID
        try_26 = F_Compile_g_throw_any(g0073.Value)
        if ErrorIn(try_26) {Result = try_26
        } else {
        v_or3 = ToBoolean(OBJ(try_26))
        if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
        } else { 
          var try_27 EID
          try_27 = F_Compile_g_throw_any(g0073.Arg)
          if ErrorIn(try_27) {Result = try_27
          } else {
          v_or3 = ToBoolean(OBJ(try_27))
          if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
          } else { 
            Result = EID{CFALSE.Id(),0}} 
          } 
    ...
    { var g0074 *Language.Do = Language.To_Do(self)
      Result = F_Compile_g_throw_any(g0074.Args.Id())
      } 
    }  else if (self.Isa.IsIn(Language.C_While) == CTRUE) { 
    { var g0075 *Language.While = Language.To_While(self)
      { 
        var v_or3 *ClaireBoolean
        
        var try_28 EID
        try_28 = F_Compile_g_throw_any(g0075.Test)
        if ErrorIn(try_28) {Result = try_28
        } else {
        v_or3 = ToBoolean(OBJ(try_28))
        if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
        } else { 
          var try_29 EID
          try_29 = F_Compile_g_throw_any(g0075.Arg)
          if ErrorIn(try_29) {Result = try_29
          } else {
          v_or3 = ToBoolean(OBJ(try_29))
          if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
          } else { 
            Result = EID{CFALSE.Id(),0}} 
          } 
    ...
    { var g0076 *Language.Construct = Language.To_Construct(self)
      Result = F_Compile_g_throw_any(g0076.Args.Id())
      } 
    }  else if (self.Isa.IsIn(Language.C_If) == CTRUE) { 
    { var g0077 *Language.If = Language.To_If(self)
      { 
        var v_or3 *ClaireBoolean
        
        var try_30 EID
        try_30 = F_Compile_g_throw_any(g0077.Test)
        if ErrorIn(try_30) {Result = try_30
        } else {
        v_or3 = ToBoolean(OBJ(try_30))
        if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
        } else { 
          var try_31 EID
          try_31 = F_Compile_g_throw_any(g0077.Arg)
          if ErrorIn(try_31) {Result = try_31
          } else {
          v_or3 = ToBoolean(OBJ(try_31))
          if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
          } else { 
            var try_32 EID
            try_32 = F_Compile_g_throw_any(g0077.Other)
            if ErrorIn(try_32) {Result = try_32
            } else {
            v_or3 = ToBoolean(OBJ(try_32))
            if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
            } else { 
              Result = EID{CFALSE.Id(),0}} 
            } 
    ...
    { var g0078 *Language.For = Language.To_For(self)
      { 
        var v_or3 *ClaireBoolean
        
        var try_33 EID
        try_33 = F_Compile_g_throw_any(g0078.SetArg)
        if ErrorIn(try_33) {Result = try_33
        } else {
        v_or3 = ToBoolean(OBJ(try_33))
        if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
        } else { 
          var try_34 EID
          try_34 = F_Compile_g_throw_any(g0078.Arg)
          if ErrorIn(try_34) {Result = try_34
          } else {
          v_or3 = ToBoolean(OBJ(try_34))
          if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
          } else { 
            Result = EID{CFALSE.Id(),0}} 
          } 
    ...
    { var g0079 *Language.Iteration = Language.To_Iteration(self)
      { 
        var v_or3 *ClaireBoolean
        
        var try_35 EID
        try_35 = F_Compile_g_throw_any(g0079.SetArg)
        if ErrorIn(try_35) {Result = try_35
        } else {
        v_or3 = ToBoolean(OBJ(try_35))
        if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
        } else { 
          var try_36 EID
          try_36 = F_Compile_g_throw_any(g0079.Arg)
          if ErrorIn(try_36) {Result = try_36
          } else {
          v_or3 = ToBoolean(OBJ(try_36))
          if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
          } else { 
            Result = EID{CFALSE.Id(),0}} 
          } 
    ...
    { var g0080 *Language.ClaireHandle = Language.To_ClaireHandle(self)
      { 
        var v_or3 *ClaireBoolean
        
        v_or3 = Core.F__I_equal_any(g0080.Test,C_any.Id())
        if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
        } else { 
          var try_37 EID
          try_37 = F_Compile_g_throw_any(g0080.Other)
          if ErrorIn(try_37) {Result = try_37
          } else {
          v_or3 = ToBoolean(OBJ(try_37))
          if (v_or3 == CTRUE) {Result = EID{CTRUE.Id(),0}
          } else { 
            Result = EID{CFALSE.Id(),0}} 
          } 
...
func F_Optimize_notOpt_Call_method (self *Language.CallMethod) EID { 
  ...
  if (self.Arg.Id() == C_Compile_m_member.Value) { 
    { var t2 *ClaireClass
      var try_1 EID
      try_1 = Language.F_static_type_any(self.Args.At(1))
      if ErrorIn(try_1) {Result = try_1
      } else {
    ...
    Result = EID{self.Args.At(0).Isa.IsIn(C_symbol).Not.Id(),0}
    }  else if ((self.Arg.Selector.Id() == C__7.Id()) || 
      (self.Arg.Selector.Id() == C_mod.Id())) { 
    Result = EID{MakeBoolean((C_integer.Id() != self.Args.At(1).Isa.Id()) || self.Args.At(1).IsInt(0)).Id(),0}
    }  else if ((C_table.Id() == self.Args.At(0).Isa.Id()) && 
    ...
    { 
      ...
      { var arg_3 *ClaireBoolean
        ...
        { var arg_5 *ClaireClass
          var try_6 EID
          try_6 = Language.F_static_type_any(self.Args.At(1))
          if ErrorIn(try_6) {try_4 = try_6
          } else {
          arg_5 = ToClass(OBJ(try_6))
          try_4 = EID{ToType(arg_5.Id()).Included(ToType(OBJ(Core.F_CALL(C_domain,ARGS(self.Args.At(0).ToEID()))))).Id(),0}
          }
          } 
        if ErrorIn(try_4) {try_2 = try_4
        } else {
      ...
      if (v_and2 == CFALSE) {Result = EID{CFALSE.Id(),0}
        ...
        v_and2 = Core.F__inf_integer(C_compiler.Safety,2)
        if (v_and2 == CFALSE) {Result = EID{CFALSE.Id(),0}
        } else { 
          Result = EID{CTRUE.Id(),0}} 
        } 
...
func F_Compile_can_throw_ask_property (p *ClaireProperty) EID { 
  ...
  { 
    ...
    v_or1 = Equal(MakeInteger(p.Open).Id(),MakeInteger(3).Id())
    if (v_or1 == CTRUE) {Result = EID{CTRUE.Id(),0}
    } else { 
      var try_1 EID
      { 
        var v_and3 *ClaireBoolean
        
        v_and3 = Core.F_not_any(ToList(C_Compile_NoErrorOptimize.Value).Memq(p.Id()).Id())
...
if ((x == C_Compile_object_I.Id()) || 
  ...
  { 
    ...
    v_and1 = C_OPT.LegalModules.Contain_ask(self.Id()).Not
    if (v_and1 == CFALSE) {g0092I = CFALSE
    } else { 
      if (C_method.Id() == x.Isa.Id()) { 
        { var g0091 *ClaireMethod = ToMethod(x)
          v_and1 = MakeBoolean((g0091.Selector.Id() != C_add_method.Id()) && ((g0091.Inline_ask.Id() == CFALSE.Id()) || 
